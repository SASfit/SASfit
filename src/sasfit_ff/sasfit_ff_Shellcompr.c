/*
 * src/sasfit_ff/sasfit_ff_Shellcompr.c
 *
...
/*
float Shellcompr(Tcl_Interp *interp,
			 float q, 
			 float Rtot, 
			 float DR, 
			 float eta_core, 
		  ...
		  float P,
			 bool  *error)
 */
scalar sasfit_ff_Shellcompr(scalar q, sasfit_param * param)
{
	scalar lRtot, lDR, leta_core, leta_sh, lx_sol, leta_sol, lcore, lshell, lsolvent, res;
...
//	lcore	 = pow(1.+P*kappa_core,1./3.);
//	lshell	= pow(1.+P*kappa_shell,1./3.);
//	lsolvent = pow(1.+P*kappa_solvent,1./3.);

	
//	leta_core = eta_core*(1.+P*kappa_core);

//	lx_sol  = 1.-(1.-x_sol)/pow(1.+P*kappa_shell,2./3);
//	leta_sh = eta_shell*(1.+P*kappa_shell);
//	lDR = DR/lshell;
//	lRtot = (Rtot-DR)/lcore+lDR;
//	leta_sol = eta_solvent*(1.+P*kappa_solvent);
...
//	leta_sol = eta_solvent*(1.0018 +P*7.4638E-5-P*P*1.0969E-8);
	...
	// return pow(Kshlin2(interp,q,lRtot,lDR,leta_core, leta_sh, lx_sol,lx_sol, leta_sol, error),2.0);
