/*
 * src/sasfit_ff/sasfit_ff_UnifiedExponetialPowerLaw.c
 *
...
scalar sasfit_ff_UnifiedExponetialPowerLaw(scalar q, sasfit_param * param)
{
	scalar x, xs, xsub, k, ks, tmp, qsub; 
	scalar G, B, Gs, Bs, Rg, Rsub, Rs, P, Psub;

	SASFIT_ASSERT_PTR(param);

	sasfit_get_param(param, 9, &G, &B, &Gs, &Bs, &Rg, &Rsub, &Rs, &P, &Psub);
	...
	return   G*exp(-x*x/3.0)   + B *exp(-xsub*xsub/3.)*pow(tmp,P) + 
		    Gs*exp(-xs*xs/3.0) + Bs*                   pow(qsub,Psub);
}
