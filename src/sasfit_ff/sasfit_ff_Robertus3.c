/*
 * src/sasfit_ff/sasfit_ff_Robertus3.c
 *
...
/*
double Robertus3(Tcl_Interp *interp,
					double q,
					double Rmean,
					double tau0,
					double Delta,
					...
					bool  *error)
*/
scalar sasfit_ff_Robertus3(scalar q, sasfit_param * param)
{
	...
	double res, eta_shell, kappa_T, rho_ref, T_ref, eta_core, x_solvent, btau, dR, sigma;
	doublereal  lRmean, lDR, leta_core, lfp, leta_sh, lx_sol, leta_sol, lcore, lshell, lsolvent, 
		kappa_core, kappa_solvent, kappa_shell;
	scalar Rmean, tau0, Delta, dTheta_dP, P, fp, L, T, Theta0, eta_solvent;

	SASFIT_ASSERT_PTR(param);

	sasfit_get_param(param, 10, &Rmean, &tau0, &sigma, &dTheta_dP, &P, &fp, &L, &T, &Theta0, &eta_solvent);
	
	eta_shell = -2.34e9;
	x_solvent = 0.47;
	eta_core = 2.41949e+010;
	...
	res = 1.0;
	btau = exp(-L*((Theta0+273.15+dTheta_dP*(P-1.))/(T+273.15)-1.))/(12.*Delta/(2.*Rmean+Delta));
	if (btau < tau0) btau=tau0;
	...
	SASFIT_CHECK_COND1((btau <= 0.0), param, "btau(%lg) <= 0",btau);
