...
#include "Burley2020Scrambling/genpoints.h"
#include <math.h>
#include "stdint.h"
...
int randomized_quasi_monte_integrate(gsl_monte_function* f, const double xl[], const double xu[],
                                     size_t dim, size_t max_calls, double max_relerr, double max_abserr,
                                     void (*rqrnd)(uint32_t , uint32_t , uint32_t , double* ), quasi_monte_state* state, double* result, double* abserr) {
    double volume = 1, mean = 0, variance_sum = 0, running_avg_abserr = GSL_POSINF;
    double* x = state->x;
    size_t n, i;
    // Check that the dimensionalities match
