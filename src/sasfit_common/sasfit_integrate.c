...
scalar sasfit_hankel(double nu, double (*f)(double, void *), double x, void *fparams) {
    ...
    switch (sasfit_get_hankel_strategy()) {
        ...
        case HANKEL_BESTLIME: {
            if (!(nu == 0.0 || nu == 1.0)) {
                sasfit_err("BESTLIME strategy is only available for Hankel transforms J0 and J1\n");
                return 0;
            }
            res = sasfit_bestlime(nu, f, x, fparams);
            if (isnan(res)) {
                sasfit_err("BESTLIME Hankel transform failed\n");
                return 0;
            }
            break;
        }
        ...
        case HANKEL_QDHT: {
            if (!(nu == 0.0 || nu == 1.0)) {
                sasfit_err("QDHT strategy is only available for Hankel transforms J0 and J1\n");
                return 0;
            }
            res = sasfit_qdht(nu, f, x, fparams);
            if (isnan(res)) {
                sasfit_err("QDHT Hankel transform failed\n");
                return 0;
            }
            break;
        }
