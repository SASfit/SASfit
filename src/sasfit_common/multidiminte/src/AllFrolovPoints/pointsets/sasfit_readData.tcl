...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	for {set i 1} {$i <= $Data(LineSkip)} {incr i} {
	   ...
	   incr lineno
	   lappend Data(Comment) "$lineno: $line"
	}
	set fieldseparator       "\t ;"
...
proc read_Ascii_Cols {filename ASCIIData args} {
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	upvar $ASCIIData Data
	if {![info exist Data]} {create_ASCIIData Data}
	if {([llength $args] == 0)} {
	...
	set fieldseparator       "\t ;"
	set fieldseparator_comma "\t ;,"		
	for {set i 0} {$i < $Data(Cols)} {incr i} {
		set Data($i) {}
	}
	while {![eof $f]} {
		gets $f line
		...
		if { $Data(Cols) <= [llength $splitline]} {
			set floats_ok yes
			for {set i 0} {$i < $Data(Cols)} {incr i} {
				if {[string is false [scan [lindex $splitline $i] "%f" Cols($i) ]]} {
					set floats_ok false
				}
			}
			if {$floats_ok} {
				for {set i 0} {$i < $Data(Cols)} {incr i} {
					lappend Data($i)   $Cols($i)
				}
			} else {
				lappend Data(Comment) $line
			}
