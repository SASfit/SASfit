...
\usepackage{graphicx}
%Figures live in docs/figures/. Both paths listed so this builds standalone
%as well as inside ozGUI_ozLib_report.tex; the trailing {./} means the move
%from docs/ into docs/figures/ required no edit to any \includegraphics.
...
The grid size is not neutral between the two transforms, and this caught us
out. A DST-I of length $N$ goes through an FFT of length $2(N+1)$, so
$N = 2^{k}-1$ makes that a power of two; a DST-IV is a length-$N$ transform
and wants $N = 2^{k}$ instead. Measured with \code{scipy.fft.dst} near
$2^{14}$:
...
$N$ & DST-I & DST-IV\\
\midrule
$16383 = 2^{14}-1$ & \textbf{0.263}\,ms & 0.308\,ms\\
$16384 = 2^{14}$   & 0.599\,ms & \textbf{0.099}\,ms\\
$16385$            & 2.497\,ms & 0.303\,ms\\
$16000$            & 2.433\,ms & 0.105\,ms\\
\bottomrule
\end{tabular}
\end{center}

So $2^{k}-1$ is $9.5\times$ faster than an awkward length for the DST-I, while
$16383 = 3\times43\times127$ is itself an awkward length for the DST-IV and
costs it a factor of $3.1$. Every grid in this package was sized $2^{k}-1$ for
the DST-I and then inherited by the type-4 path, so every type-4 \emph{timing}
was taken on a grid that penalises it threefold. That explains a result
recorded earlier as inexplicable --- the second-order transform benchmarking
slower than the first-order one at fine grids, which was attributed at the
time to the FFT backend. It was the grid.

Accuracy is unaffected: both lengths are valid transforms, and the error
comparison between them stands as measured. \code{bestGridSize(n, type)}
returns the appropriate length, and \code{checkTransformAlignment()} now says
so when a type-4 solve is run on a type-1 grid.
...
so adding one is a single line and an import failure costs only that tab. The
generic tab replaced the two analytic special-case tabs that preceded it
(\code{polydisperse\_yukawa\_tab} and \code{robertus\_shs\_tab}); both modules
remain importable, and restoring either is one line here.

The tabs are numbered in their labels --- \code{0: OZ solver},
\code{1: Polydisperse (any potential)}, \code{2: RY Polydisperse Yukawa},
\code{3: Mixture validation} --- and the number is taken from each widget's
own \code{SESSION\_TAB\_INDEX} rather than from its position in the registry.
The two can disagree: if a tab fails to import, the remainder shift up, and
the label must continue to match what the tab's own \emph{files} claim.

\paragraph{Sessions are saved per tab, not globally.} Each tab writes its own
file with the extension carrying its index (\code{.oz1}, \code{.oz2},
\code{.oz3}), holding the computed runs, every input field, which parameters
were free, the measured data \emph{including} the resolution column, and the
last fit result. Saving the data with the file rather than a path to it costs
little and survives the data being moved.

A combined all-tabs save was considered and rejected. The tabs hold different
models with different parameter sets, so one file would be a bag of unrelated
sessions and loading it would overwrite work in tabs the user was not thinking
about. Per-tab, a load affects only what is in front of them.

That makes it possible to open a file in the wrong tab, so the tab NAME is
written into every file and checked on load; a mismatch is refused outright.
Without that check the load would set whichever entries happened to share a
name and silently ignore the rest --- a state half one model and half another,
reported as success. The extension is a convenience for the file manager; the
name inside is the guard, and it survives the tabs being reordered.

Both modelling tabs follow the same pattern: a parameter panel; plot tabs for
...
\paragraph{The generic tab's model.} What is computed is
\begin{equation}
  I(Q) \;=\; \mathrm{scale}\times I_{\mathrm{model}}(Q) \;+\;
  \mathrm{background},
\end{equation}
with the scale and background entering linearly and therefore solved exactly
by weighted linear least squares at every iteration rather than handed to the
nonlinear optimiser --- which would add the two most strongly correlated
parameters (scale against volume fraction, background against everything at
high $Q$) for no gain. Both appear in the interface so the solved values are
visible, and either can be held fixed when known independently. A scale far
from its expected value is a diagnostic in itself: since
$I = n\langle|F|^2 S\rangle$, the prefactor is the particle number density
together with the conversion implied by the chosen units of $Q$ and of the
scattering length density.

The form factor may be a sphere or a core--shell particle. The core--shell
form is polydisperse in the CORE radius with a constant shell thickness
$\Delta R$, so that $R_{\mathrm{outer}} = R_{\mathrm{core}} + \Delta R$ and the
interaction diameter is $2(R_{\mathrm{core}} + \Delta R)$. That is a different
model from one polydisperse in the outer radius: with a constant shell the
outer distribution is the core distribution \emph{shifted} rather than scaled,
so its relative width is narrower by $R_c/(R_c+\Delta R)$, and fitting the
wrong one biases both the recovered polydispersity and the effective volume
fraction. It is parametrised by $\Delta R$ rather than by a core/outer ratio
because $\Delta R$ is directly comparable with an independently known layer
thickness.

The radius, relative width, volume fraction, shell thickness, both scattering
length densities, the closure parameter and the potential's own arguments may
all be fitted. The scattering length densities are deliberately unbounded:
their units are the user's choice and span many orders --- a typical value in
$\mathrm{cm}^{-2}$ is $\sim10^{10}$ --- and, more importantly, an opposite sign
between core and shell must be an outcome of the fit rather than something the
bounds impose. Scale, core and shell density cannot be fitted together, since
the intensity depends on the scale times the squared contrast; the interface
refuses that combination rather than letting the optimiser wander along a flat
valley and report a converged fit whose parameters are meaningless.

The size distribution is selectable among Schulz, Gaussian, log-normal and
Weibull. The quadrature nodes are moment-matched for each, so the choice costs
nothing numerically, but it changes the tail --- and $\langle\sigma^6\rangle$
governs $I(Q\to0)$. Two distributions with the same mean and relative width
can differ by tens of per cent in the computed intensity, which is a reason to
try more than one on measured data rather than to assume Schulz.

\paragraph{Resolution.} Where a fourth data column supplies $\mathrm{d}Q$, the
smearing is applied to every curve or to none: the exact result and all six
approximations are evaluated on the kernel's extended grid and smeared back
onto the measured $Q$ points. Smearing only the exact curve would be worse
than not smearing at all, since part of the apparent difference between the
schemes would then be instrumental rather than a property of the
approximation --- and precisely near a form-factor minimum, where smearing
fills the minimum in and where the schemes differ most. $S(Q)$ and the partial
structure factors are left unsmeared: they are diagnostics rather than
observables.
...
measured at $\gamma > 0$. See the remark below: for BPGG and Duh--Haymet
this range concealed a missing $\gamma < 0$ branch.}
\begin{tabular}{lc}
\toprule
...
\begin{remark}
The agreement above was measured at $\gamma > 0$ only, and for BPGG and
Duh--Haymet that hid a defect. Table~I of the accompanying paper gives
$b = -\gamma^2/2$ for $\gamma < 0$ for BPGG, MV, DH and CG; of these only CG
implemented it here. BPGG instead continued its power law to negative
$\gamma$ through $\mathrm{sign}(\cdot)|\cdot|^{1/s}$, which produces neither
an overflow nor a NaN --- so nothing appeared wrong --- but is a numerical
continuation rather than the published closure. Duh--Haymet was worse: its
rational form is singular at negative $\gamma^*$, with poles at
$\gamma^* = -9/7$ and $\gamma^* \approx -0.6$ and $\max|B| = \infty$.

Both now take the published branch. The effect on results is small at low
density and grows with coupling: for hard spheres, BPGG changes by
$1\times10^{-5}$ at $\varphi = 0.1$ and Duh--Haymet by $2\times10^{-3}$ at
$\varphi = 0.4$. A Yukawa fluid at $\varphi = 0.2$ is unchanged to
$7\times10^{-15}$, which is the expected signature of a branch that fires
only where it should. For $\gamma^* \ge 0$ no pole is possible in the
Duh--Haymet form, since $7\gamma^*+9 > 0$ and $(5\gamma^*+11)\gamma^* \ge 0$,
so the branch removes the singularities as well as matching the publication.

The general point is that agreement to $10^{-16}$ over a restricted range of
an argument says nothing about the rest of it. Cross-validation is only as
good as the states at which it is performed.
\end{remark}
...
%======================================================================
\section{Solver reliability: multiple solutions and verification}

The work described above assumed throughout that a converged solve returns
the right answer. That assumption does not hold in general, and the failure
is not a defect in any particular solver.

\subsection{The Ornstein--Zernike equation has several solutions}

The number of solutions of the OZ equation with a given closure changes with
density and temperature. Beardmore, Peplow \& Bresme (SIAM J.\ Sci.\ Comput.
\textbf{29}, 2442, 2007) analysed the bifurcation structure for a
Lennard-Jones fluid under HNC: a subcritical isotherm carries at least one
\emph{fold bifurcation} in density, and pairs of branches collide at what
they call pseudospinodals --- one branch with positive compressibility, the
other negative and therefore unphysical. They state that this explains
``previous inconclusive attempts to compute solutions using Newton--Picard
methods that are popular in the physical chemistry literature'', which is
precisely what this package uses. Peplow \emph{et al.} (Phys.\ Rev.\ E
\textbf{74}, 046705, 2006) show further that solutions exist inside the
so-called no-solution region which direct iteration cannot reach at all, and
recover them by pseudo-arclength continuation.

We reproduce this. At $k_BT/\varepsilon = 1.25$, $\rho\sigma^3 = 0.57$ ---
well inside their problem region, and at lower temperature and roughly twice
the density of their published examples --- two of the five available solvers
return $g_{\max} = 1.13$ and $1.43$ against the correct $2.16$, finite,
converged, and with no error raised. The $1.43$ root has
$\min S(Q) = -46.5$: it is not a failed calculation but a converged solution
on the negative-compressibility branch the bifurcation analysis identifies.

\subsection{What the library does about it}

\paragraph{The physicality test is the discriminator, not a diagnostic.}
Since a structure factor is a variance it cannot be negative, and
$\min S(Q) \ge 0$ is the same criterion the bifurcation studies use to reject
branches. It is applied unconditionally. A user who disabled it would obtain
a converged, finite, entirely wrong answer.

\paragraph{Verification by an independent solver, on by default.}
\code{ozLib.solve()} now takes \code{verify=True}: the solve is repeated with
an independent solver from a \emph{cold start} and the result returned only if
the two agree on $S(Q)$ to $10^{-3}$. Disagreement raises, naming both solvers
and the size of the difference. \code{ozLib.solveWithConsensus()} is the more
general form, running any number of solvers and requiring at least two to
agree. Cost is two solves --- 0.14\,s against 0.05\,s for hard spheres.

The verifier is chosen for \emph{independence} rather than speed. Plain
Picard iteration carries no acceleration and no history of previous iterates,
so it cannot share an acceleration failure mode with the primary solver;
Biggs--Andrews, which does keep a history, is the fallback when Picard fails.

\paragraph{The cold start cannot be optimised away.} Warm-starting the
verifier from the first answer is about three times faster and destroys the
check. A wrong root is still a root: started from the erroneous $1.43$ above,
the otherwise reliable solver stays at $1.43$, the two ``agree'', and the
wrong answer is certified. The question the check must ask is whether
independent searches from different starting points reach the same basin.

\paragraph{Limitation.} Detecting disagreement is not the same as resolving
it. Where a branch must be \emph{followed} through a fold, pseudo-arclength
continuation is the appropriate tool and is not implemented here.

\subsection{Solver behaviour, measured}

Measured over five test cases (hard spheres at $\varphi = 0.3$ and $0.45$,
Lennard-Jones, square well, Yukawa):
...
Solver & solves & note\\
\midrule
Anderson acceleration & 5/5 & correct, fastest (0.12\,s total)\\
Biggs--Andrews & 5/5 & correct\\
Picard iteration & 5/5 & correct\\
\code{scipy} Anderson & 5/5 & \textbf{wrong root} for Lennard-Jones\\
\code{scipy} Newton--Krylov & 4/5 & fails Lennard-Jones; slowest\\
\bottomrule
\end{tabular}
\end{center}

Two corrections to earlier statements in this report follow from these
measurements. First, Anderson acceleration does \emph{not} require Picard
pre-conditioning: it converges from $\Gamma = 0$ at every state point tested,
and no pre-conditioning step exists anywhere in the code. Second, the Picard
fallback was described as ``damped'' when it was not; it is now implemented in
the averaged (Mann, or Krasnoselskii--Mann) form
$\Gamma_{n+1} = (1-\alpha)\Gamma_n + \alpha T(\Gamma_n)$ with $\alpha = 1$ as
the default, so existing results are unchanged, and \code{mannAlpha} available
to damp. The averaging matters: the Lennard-Jones case above diverges at
$\alpha = 1$ and converges at $\alpha = 0.5$ on one discretisation, while on
another the two exchange roles.

A third correction concerns cost rather than correctness. \code{scipy}'s
Anderson implementation was the slowest solver available --- 30\,s where
others took 0.5\,s --- purely because its initial inverse-Jacobian scale was
left unset. Setting it makes that solver the fastest, a factor of 15 to 66,
with the fixed point unchanged to $10^{-9}$. Note that this is purely a cost
effect: with the scale set, that solver converges to the same physical root
as the other fixed-point methods on every case tested, so speed and branch
selection are independent questions.

\subsection{A converged flag is a claim; the residual is evidence}

Throughout this section $\Gamma(r) = h(r) - c(r)$ denotes the indirect
correlation function --- the quantity every solver here actually iterates on,
rather than $g(r)$ or $S(Q)$, because it is smooth across the hard core where
$h$ and $c$ are not. $h(r) = g(r) - 1$ is the total correlation function and
$c(r)$ the direct correlation function.

Writing the closure as $c = \mathcal{C}[\Gamma]$ and the Ornstein--Zernike
relation in Fourier space as $\hat\Gamma = \rho\hat c^{\,2}/(1 - \rho\hat c)$,
the two compose into a single \emph{fixed-point map}
\begin{equation}
 T(\Gamma) \;=\; \mathcal{F}^{-1}\!\left[
   \frac{\rho\,\hat c^{\,2}}{1 - \rho\,\hat c}\right],
 \qquad \hat c = \mathcal{F}\big[\mathcal{C}[\Gamma]\big],
 \label{eq:fixedpointmap}
\end{equation}
where $\mathcal{F}$ is the three-dimensional radial Fourier transform and
$\rho$ the total number density. A solution of the Ornstein--Zernike system
is exactly a fixed point of this map, $T(\Gamma) = \Gamma$, and the
\emph{residual}
\begin{equation}
 \|T(\Gamma) - \Gamma\|_2
 \label{eq:residual}
\end{equation}
is therefore zero at a solution and nowhere else. For a multicomponent system
the same holds with $\Gamma$, $c$ and $h$ replaced by $p\times p$ matrices
$\Gamma_{ij}$, $c_{ij}$, $h_{ij}$, the scalar division by a matrix inverse,
and $\rho$ by $\sqrt{\rho_i\rho_j}$.

The distinction that matters below is that a solver's own convergence flag
reports whether \emph{its own} stopping criterion was met --- which may test
the change between iterates, or their norms, or an internal estimate --- while
\eqref{eq:residual} tests whether the answer solves the equations. The two
are not the same, and the difference is not academic.

The two situations above look identical from the outside --- solvers
disagreeing, every one of them reporting success --- and are distinguished
only by evaluating \eqref{eq:residual} at each claimed solution. Measured
across all nine available solvers on a $4095$-point grid:

\begin{center}\small
\begin{tabular}{lll}
\toprule
State & Outcome & Residual\\
\midrule
Lennard-Jones, $\varepsilon = 0.8$, & 5 fixed-point solvers, $g_{\max} = 2.1636$ & $< 10^{-11}$\\
$\varphi = 0.3$, HNC & 4 Newton--Krylov, $g_{\max} = 1.13$--$1.22$ & $< 10^{-11}$\\
\midrule
BPGG $\alpha = 0.5$, & 8 solvers, $g_{\max} = 5.2528$ & $< 10^{-12}$\\
$\varphi = 0.4$ & Picard, $g_{\max} = 3.0046$ & $\mathbf{19.4}$\\
\bottomrule
\end{tabular}
\end{center}

The Lennard-Jones state has genuinely distinct solutions: all nine points are
fixed points, and the Newton--Krylov family simply lands on branches with
$\min S(Q) \approx -38$, which are unphysical but real. The BPGG state has
ONE solution; the ninth value is not a fixed point at all.

The cause of that last case was a defect in the Picard convergence test,
which compared successive \emph{norms} of $\Gamma$ rather than the change in
$\Gamma$. Two different vectors of similar norm pass such a test, so an
iterate still moving but whose norm has momentarily plateaued is reported as
converged. It now tests the increment, and exposes a \code{converged}
attribute that defaults to false, since exhausting the iteration limit
previously returned the last iterate with nothing a caller could check.

Two practical consequences. First, recompute the residual after every solve;
it costs one operator evaluation and is the only thing that distinguishes the
two rows above. Second, when using one solver to verify another, note that
\code{Picard iteration} and \code{Biggs--Andrews} converge to residuals of
$\sim 5\times10^{-11}$ on hard spheres where the others reach
$10^{-13}$--$10^{-14}$; a verifier's own tolerance sets the floor on the
disagreement it can detect.

Where the question is not whether a point is a solution but whether OTHER
solutions exist, the appropriate tool is deflation
\citep{FarrellBirkissonFunke2015}. Given a known root $r$ of the residual
$F(\Gamma) = T(\Gamma) - \Gamma$, one forms the \emph{deflated residual}
\begin{equation}
 G(\Gamma) \;=\; M(\Gamma; r)\, F(\Gamma),
 \qquad
 M(\Gamma; r) \;=\; \frac{1}{\|\Gamma - r\|^{p}} + \alpha ,
 \label{eq:deflation}
\end{equation}
and re-solves \emph{from the same initial guess}. The exponent $p$ (here
$p = 2$) controls how sharply the known root is excluded, and the shift
$\alpha$ (here $\alpha = 1$) is essential rather than cosmetic: with
$\alpha = 0$ the deflated residual tends to zero as
$\|\Gamma - r\| \to \infty$, so a solver can report convergence having simply
run away. Because $M$ is finite and non-zero at every other root, those are
left reachable, and repeating with one factor per known root discovers them
in turn.

The method is not a guarantee --- it finds distinct solutions where they
exist and the solver converges, but does not certify that none remain --- and
the authors note it is sensitive to $\alpha$, with no a priori guidance.
Applied to the BPGG state above, deflating the single root yields nothing
further, consistent with there being only one.

\section{Fitting measured data}

What is fitted is
\begin{equation}
  I(Q) \;=\; \mathcal{R}\!\left[\,\mathrm{scale}\times I_{\mathrm{model}}(Q)
  \;+\; A\,Q^{-4+d}\,\right] \;+\; \mathrm{background},
  \label{eq:fitmodel}
\end{equation}
where $\mathcal{R}$ is the resolution smearing. The power-law term describes
the sloping background that real curves frequently carry --- Porod's $Q^{-4}$
for a smooth sharp interface, shallower for a rough or fractal one. With only
a flat constant available the fit absorbs that slope into the shape
parameters, where it does real damage.

Note which terms are inside $\mathcal{R}$. The power law is scattering and is
smeared; the incoherent background is flat and unaffected by smearing, so it
is added afterwards.

\paragraph{Linear terms are solved, not searched.} The scale, the constant
and the power-law amplitude all enter \eqref{eq:fitmodel} linearly, so at
every iteration they are obtained exactly by weighted linear least squares
over whichever of them are free, while the nonlinear optimiser sees only the
shape parameters and the exponent $d$. Giving them to the optimiser instead
would add the three most strongly correlated quantities in the problem ---
scale against volume fraction, background against everything at high $Q$, the
power law against both --- to a search that gains nothing by including them.
Enabling the power law therefore costs ONE extra fitted parameter, not two.
Any of the three may also be held fixed, in which case the remainder are
still solved exactly; a held term simply moves to the right-hand side.

\paragraph{The resolution kernel is Rician, not Gaussian.} A small-angle
instrument broadens $\mathbf{Q}$ in the detector plane, in two dimensions, and
the data are then radially averaged. For isotropic Gaussian broadening of
width $\sigma$ the distribution of the \emph{magnitude} about a true $Q_0$ is
\begin{equation}
  ...
  \label{eq:rician}
\end{equation}
with $I_0$ the modified Bessel function of the first kind. A Gaussian in $Q$
is the $Q_0/\sigma \gg 1$ limit of this and is adequate wherever
$\mathrm{d}Q/Q$ is small. It is not always small: for the spherical-shell data
used to develop this, $\mathrm{d}Q/Q$ reaches $0.55$ at the lowest measured
point, where a Gaussian places $44\,\%$ of that point's kernel weight below
$Q_{\min}$ and some of it at \emph{negative} $Q$. The leading factor of $Q$ in
\eqref{eq:rician} suppresses the origin, reducing that to $34\,\%$ and
excluding negative $Q$ by construction. Both forms reproduce the unsmeared
curve to $2\times10^{-16}$ as $\mathrm{d}Q\to0$, which is the test that
catches a mis-normalised kernel. \eqref{eq:rician} is evaluated through the
exponentially scaled $\tilde{I}_0(x) = e^{-|x|}I_0(x)$, since the argument
reaches several hundred at these widths and $I_0$ overflows a double long
before the prefactor cancels it.

\paragraph{The power law must not be extrapolated to $Q\to0$.} $Q^{-4+d}$
diverges at the origin while the smearing kernel reaches below the data, so
something has to bound it. Left unbounded the result is set by whatever guard
prevents division by zero rather than by the physics: with a Gaussian kernel
the smeared power law at the lowest measured point came out $6.6\times10^{5}$
times the unsmeared value, and moving that guard from $Q_{\min}/1000$ to
$Q_{\min}/10$ changed the answer by five orders of magnitude. The term is
therefore held constant below a $Q$ the user sets. Choosing the lowest
measured $Q$ suppresses the extrapolation entirely; the default is far below
it, which keeps the power law extending into the unmeasured region. That is a
physical judgement --- whether the scattering really continues to rise below
the window --- and is exposed rather than decided in the code. It is worth
fitting both ways: if $A$ and $d$ are stable the choice does not matter, and
if they swing then the lowest points are driving the result.

\paragraph{Linking the interaction range to the shell.} For the square-well
and sticky-sphere potentials the second argument is the well WIDTH as an
absolute length, directly comparable with a core--shell particle's shell
thickness. Where the attraction comes from the surface layer the two are the
same physical quantity, and fitting them independently invites them to trade
against one another. They can therefore be linked as $\delta = c\,\Delta R$,
with $\delta$ recomputed from the current $\Delta R$ at every iteration and
removed from the fitted set. The constant $c$ may itself be fitted, bounded
between $0$ and $2$: $c=1$ if the range is the layer, $c=2$ if two layers
overlap when particles touch.

\paragraph{The contrast degeneracy is removed, not refused.} The intensity
depends on
the scale times the squared contrast, so the absolute scattering length
densities carry no information the scale does not. The fitted contrast
parameter is therefore the RATIO $\rho_{\mathrm{shell}}/\rho_{\mathrm{core}}$,
which the interface exposes as its own field alongside the two densities;
editing any of the three updates the others. Fitting the ratio also makes
the parameter refine at all: in $\mathrm{cm}^{-2}$ a scattering length
density is $\sim10^{10}$, and \code{least\_squares} satisfies its gradient
test after a single evaluation at that magnitude, returning the starting
value with a meaningless uncertainty. A ratio of order one has no such
problem. Its bounds span both signs and are wide, so that an opposite sign
between core and shell is an outcome of the fit rather than an imposition.

\paragraph{Ill-conditioning in the linear solve.} The three linear
coefficients are obtained from a weighted least-squares problem whose design
columns can differ in magnitude by twenty orders: a model built from
scattering length densities in $\mathrm{cm}^{-2}$ carries contrast squared
and so has magnitude $\sim10^{20}$, while the constant background column is
unity. \code{numpy.linalg.lstsq} discards singular values below
$\max(M,N)\epsilon$, so at a condition number of $10^{20}$ the background and
power-law coefficients are truncated to EXACTLY zero and the solve reports
success. The symptom is a fit returning $\mathrm{background} = 0$ and
$A = 0$ that a hand-chosen background of $0.5$ and amplitude of
$2\times10^{-3}$ beats by $30\,\%$ in $\chi^2$ --- which looks like a broken
optimiser and is a discarded singular value. Equilibrating the columns to
unit norm before the solve and rescaling afterwards restores all three
coefficients to five figures, and leaves well-conditioned problems
unchanged.

\paragraph{Bounds are shown, editable, and not silently widened.} Every
fittable parameter carries its own \emph{vary} checkbox and a pair of bound
fields beside its value, greyed together when the parameter does not apply
to the current model. Showing the bounds matters even unedited: a parameter
pinned at a limit is a result --- the data want to go where the model
forbids --- and that is invisible if the limit is. Editing them is how
independent knowledge enters: a radius from electron microscopy or a volume
fraction from weighing constrains the fit far better than a generic range.

The built-in limits are treated as the maximum DOMAIN rather than as
defaults to be overridden, so an edited bound may narrow the range and not
widen it, and a value outside its own bounds is refused rather than clipped.
Both are reported as errors. Silently moving a starting value to a boundary,
or honouring a limit the user typed only when it happens to be convenient,
makes a constraint into a suggestion --- and a limit that looks respected and
is not is worse than no limit at all.

\paragraph{Optimisers.} The default is \code{scipy.optimize.least\_squares},
a bounded trust-region method that converges in some thirty model
evaluations. Since one evaluation is a full Ornstein--Zernike solve ---
$0.05$\,s for one component, $2.5$\,s at seven size classes --- the
population methods available through \code{bumps} (differential evolution,
DREAM) are hours rather than minutes and are not everyday alternatives. They
earn their place in two situations: DREAM samples a posterior and returns a
correlation matrix, which is the only honest way to see how many of eleven
nominally free parameters the data actually determine; and a population
method can cross between minima that a local one cannot, which matters when
a sign or a branch is in question. A multi-start option answers the same
question for about three hundred evaluations rather than several thousand.

\code{NLopt} is offered for one reason only: constraints that RELATE several
parameters, which no box bound can express. The concrete one here is that
the shell must fit inside the core --- and not inside the mean core but the
smallest, since the distribution has width $s$ and the smallest class has
radius about $R(1-ns)$:
\begin{equation}
  \Delta R \;<\; R\,(1 - n\,s).
\end{equation}
Without it an unconstrained fit on synthetic data drove the shell to $0.84$
against a limit of $0.63$ --- thicker than the core it sits on --- while the
constrained fit found the right answer with a LOWER $\chi^2$. AUGLAG wraps a
derivative-free inner method, so the constraint costs no extra evaluations
per step.

\paragraph{Warm starting.} Each solve may be seeded with the previous
converged $\Gamma$ rather than started from zero. Measured at 1.2 times
faster with 20\,\% fewer solver iterations and an identical fit result, and
the gain grows with the cost of a single solve. It also keeps the fit on one
branch of the closure equations, which makes the residual smooth in the
parameters where a cold start can land on a different branch between
adjacent iterations. It is off by default, because it can equally carry a
fit ONTO a wrong branch and hold it there; the residual check and the
$\min S(Q) \ge 0$ screen bound that risk but do not remove it.

\paragraph{What a session file holds.} Settings, the measured data
including its resolution column, which parameters were free, any edited
bounds, and the last fit result --- everything needed to resume. It does NOT
hold the computed runs: those are regenerated by pressing Compute, and
storing them was expensive out of all proportion. One partial structure
factor is shaped $(n_Q, p, p)$, which at 200 $q$-points and 40 classes is
320\,000 values per run; thirty runs of that produced a 196\,MB file, large
enough to fill a disk after a few saves. Written without the runs and
indented for legibility the same session is 23\,kB.

Edited bounds travel with the file deliberately. A limit narrowed from an
independent measurement is knowledge about the sample rather than a
preference, and losing it on reload would quietly return the fit to the
generic defaults.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{gui_controls.png}
\caption{The parameter panel. Each fittable quantity carries its value, a
\emph{vary} checkbox and its own pair of bounds on one row, so nothing has
to be matched against a separate list. The bounds are editable: the built-in
limits shown here are the maximum \emph{domain}, which an independent
measurement may narrow but not widen. Parameters that do not apply to the
current model are greyed across all four fields --- the interaction-range
ratio $c$ here, because the $\delta = c\,\Delta R$ link is switched off ---
rather than hidden, so the layout does not reflow as options are toggled.
The divider between panel and plots is draggable.}
\label{fig:guicontrols}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{gui_iq.png}
\caption{The $I(Q)$ tab after a calculation. The exact multicomponent result
is drawn with the six approximation schemes on the same axes and, where data
are loaded, against the measurement --- which is the point of computing them
together: the error of each scheme is read off the plot at the state point
being fitted rather than assumed from the literature. The legend carries the
reduced $\chi^2$ of both the fit and the computed curve, together with the
solved scale, background and power-law amplitude.}
\label{fig:guiiq}
\end{figure}
