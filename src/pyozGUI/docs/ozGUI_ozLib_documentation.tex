...
\usepackage{graphicx}
%Figures live in docs/figures/. Both paths are listed so this file builds
%standalone AND as part of ozGUI_ozLib_report.tex, whose working directory
%is the same but whose graphicspath would otherwise be the only one
%declared. The trailing {./} keeps any figure still loose in docs/ working,
%so the move into figures/ needed no edit to a single \includegraphics.
