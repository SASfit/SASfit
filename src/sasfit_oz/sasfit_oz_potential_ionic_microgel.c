...
double U_Ionic_Microgel(double r, double T, double *p) {
   ...
   {p1=(1-exp(-kp*r)+0.5*pow(kp,2)*pow(r,2)+(pow(kp,4)*pow(r,4)/24))*(1-(4/(pow(kp,2)*pow(sigma,2))));
    ...
    return EPSILON*T*kb*pow(1-(r/sigma),2.5)+((2*pow(Z,2)*pow(el,2))/(ED*sigma))*(1.2-2*pow(r/sigma,2)+1.5*pow(r/sigma,3)-0.2*pow(r/sigma,5))-phiind*(72*pow(Z,2)*pow(el,2)/(ED*pow(kp,4)*pow(sigma,4)*r));
        }
   else  return (144*pow(Z,2)*pow(el,2)/(ED*pow(kp,4)*pow(sigma,4)))*pow(cosh(0.5*kp*sigma)-(2*sinh(0.5*kp*sigma)/(kp*sigma)),2)*exp(-kp*r)/r;
    }
