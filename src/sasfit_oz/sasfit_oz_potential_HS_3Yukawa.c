...
#define LAMBDA1 p[2] // screening length of first Yukawa contribution
#define K2      p[3] // depth of second Yukawa contribution
#define LAMBDA2 p[4] // screening length of second Yukawa contribution
#define K3      p[5] // depth of third Yukawa contribution
#define LAMBDA3 p[6] // screening length of third Yukawa contribution
...
double U_HS_3Yukawa(double r, double T, double *p) {
    ...
    if (r<sigma) {
        ...
        Y1 = -K1*exp(-(r-sigma)/sigma/LAMBDA1);
        Y2 = -K2*exp(-(r-sigma)/sigma/LAMBDA2);
        Y3 = -K3*exp(-(r-sigma)/sigma/LAMBDA3);
...
double U_R_HS_3Yukawa(double r, double T, double *p) {
    ...
    if (r<sigma) {
        ...
        if (K1<0) Y1 = -K1*exp(-(r-sigma)/sigma/LAMBDA1); else Y1=0;
        if (K2<0) Y2 = -K2*exp(-(r-sigma)/sigma/LAMBDA2); else Y2=0;
        if (K3<0) Y3 = -K3*exp(-(r-sigma)/sigma/LAMBDA3); else Y3=0;
        return kb*T*sigma/r*(Y1+Y2+Y3);
    }
}
