/*
 * src/sasfit_sq/sasfit_sq_ThinCylindricalShell.c
 *
...
scalar sasfit_sq_ThinCylindricalShell_core(scalar x, sasfit_param * param)
{
        scalar Q, P, LNdistr;
...
scalar sasfit_sq_ThinCylindricalShell(scalar q, sasfit_param * param)
{
	...
	if ((q != q_old) || (R0 != R0_old) || (sigma != sigma_old)) 
	{
		...
		if (sigma == 0) 
		{
			...
			res = sasfit_integrate(Rstart, Rend, sasfit_sq_ThinCylindricalShell_core, &subParam);
