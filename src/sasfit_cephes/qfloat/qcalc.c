...
if( pline == line )
	...
	if( takptr < 0 )
		{	/* no take file active: prompt keyboard input */
		  /* Thanks to John Dahlstrom <jd@freeshell.org>
		     for a readline fix.  */
#if !USE_READLINE
		printf("* ");
#endif
...
/* Jacobian elliptic functions ellpj (k, m)  */
int zqellpj(k, m, y)
QELT *k, *m, *y;
{
  QELT sn[NQ], cn[NQ], dn[NQ], ph[NQ];

  qellpj( k, m, sn, cn, dn, ph );

  qtoasc( sn, str, ndigits );
  printf ("sn = ");
  printf( "%s\n", str );

  qtoasc( cn, str, ndigits );
  printf ("cn = ");
  printf( "%s\n", str );

  qtoasc( dn, str, ndigits );
  printf ("dn = ");
  printf( "%s\n", str );

  qtoasc( ph, str, ndigits );
  printf ("phi = ");
  printf( "%s\n", str );
