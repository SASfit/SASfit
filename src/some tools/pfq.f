...
C     
      ...
      FUNCTION PFQ (A,B,IP,IQ,Z,LNPFQ,IX,NSIGFIG,IERR,ERRSTR)              
      
      INTEGER LNPFQ,IX,IP,IQ,NSIGFIG, IERR   
	...
	IERR=0
      ...
      IF ((LNPFQ .NE. 0) .AND. (LNPFQ .NE. 1)) THEN 
         WRITE (ERRSTR,*) ' ERROR IN INPUT ARGUMENTS: LNPFQ /= 0 OR 1' 
         IERR=1
         PFQ=999.99
         RETURN
      END IF
      IF ((IP .GT. IQ) .AND. (ABS(Z) .GT. ONE) .AND. IP .GT. 2) THEN
         WRITE (ERRSTR,300) IP,IQ,ABS(Z)
         IERR=1
         PFQ=999.99
         RETURN
      END IF
...
*
      IF (IP .EQ. 2 .AND. IQ .EQ. 1 .AND. ABS(Z) .GT. ONE) THEN
         ...
         A1(2) = ONE-B(1)+A(1)
         B1(1) = ONE-A(2)+A(1)
         GAM1 = CGAMMA(B(1),LNPFQ,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
         GAM2 = CGAMMA(A(2)-A(1),LNPFQ,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
         GAM3 = CGAMMA(A(2),LNPFQ,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
         GAM4 = CGAMMA(B(1)-A(1),LNPFQ,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
         GAM5 = CGAMMA(A(1)-A(2),LNPFQ,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
         GAM6 = CGAMMA(A(1),LNPFQ,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
         GAM7 = CGAMMA(B(1)-A(2),LNPFQ,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
         HYPER1=HYPER(A1,B1,IP,IQ,Z1,LNPFQ,IX,NSIGFIG,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
         ...
         A1(2) = ONE-B(1)+A(2)
         B1(1) = ONE-A(1)+A(2)
         HYPER2=HYPER(A1,B1,IP,IQ,Z1,LNPFQ,IX,NSIGFIG,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
...
*     
 ...
 20      CONTINUE
         ...
         IF (IERR .EQ. 1) THEN
            ...
            RETURN
         END IF
         GAM2=CGAMMA(B(1)-A(1)-A(2),LNPFQ,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
         GAM3=CGAMMA(B(1)-A(1),LNPFQ,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
         GAM4=CGAMMA(B(1)-A(2),LNPFQ,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
         GAM5=CGAMMA(A(1)+A(2)-B(1),LNPFQ,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
         ...
         IF (IERR .EQ. 1) THEN
            ...
            RETURN
         END IF
         GAM7=CGAMMA(A(2),LNPFQ,IERR,ERRSTR)
         IF (IERR .EQ. 1) THEN
            PFQ=999.99
            RETURN
         END IF
     ...
     :        GAM1*GAM5*HYPER2/(GAM6*GAM7)
      ...
      END IF
 30   CONTINUE 
      PFQ=HYPER(A,B,IP,IQ,Z,LNPFQ,IX,NSIGFIG,IERR,ERRSTR)
      IF (IERR .EQ. 1) THEN
         PFQ=999.99
         RETURN
      END IF
...
C     ****************************************************************  
      ...
      FUNCTION HYPER (A,B,IP,IQ,Z,LNPFQ,IX,NSIGFIG,IERR,ERRSTR) 
      ...
      DIMENSION QR1(-1:LENGTH),QR2(-1:LENGTH),
     :     QI1(-1:LENGTH),QI2(-1:LENGTH)         
      DIMENSION A(IP),B(IQ),AR(10),AI(10),AR2(10),AI2(10),                
     :     CR(10),CI(10),CR2(10),CI2(10)                           
      DIMENSION WK1(-1:LENGTH),WK2(-1:LENGTH),WK3(-1:LENGTH),
     :     WK4(-1:LENGTH),WK5(-1:LENGTH),WK6(-1:LENGTH)
      DOUBLE PRECISION AR,AI,CR,CI,XR,XI,SUMR,SUMI,CNT,SIGFIG
      DOUBLE PRECISION NUMR,NUMI,DENOMR,DENOMI,RMAX,MX1,MX2,CREAL
...
*     
     ...
     :        (CI(I1) .EQ. ZERO) .AND. (CI2(I1) .EQ. ZERO)) THEN
            WRITE (ERRSTR,304) I1
            HYPER=DCMPLX(999.99D9,999.99D9)
            IERR=1
            RETURN
            STOP
         END IF
...
*     
      ...
      ACCY = TEN**(-NSIGFIG)                                         
      L=IPREMAX(A,B,IP,IQ,Z,IERR,ERRSTR)
      IF (L .EQ. 1) GO TO 110
*     
...
C     ****************************************************************  
      ...
      SUBROUTINE ARYDIV(AR,AI,BR,BI,C,L,LNPFQ,RMAX,IBIT,IERR,ERRSTR)                 
      
      INTEGER L,IBIT,REXP,IR10,II10,LNPFQ,ITNMAX,IERR
      CHARACTER*256 ERRSTR
      COMPLEX*16 C,CDUM                                                 
...
C     ****************************************************************  
      ...
      SUBROUTINE CONV21(CAE,CN,IERR,ERRSTR)                                         
...
*     CN=DCMPLX(TENMAX,TENMAX)
         WRITE (ERRSTR,300) ITNMAX
         CN=DCMPLX(999.99D9,999.99D9)
         IERR=1
         RETURN
         STOP
...
C     ****************************************************************  
      ...
      FUNCTION IPREMAX(A,B,IP,IQ,Z,IERR,ERRSTR)
      COMPLEX*16 A,B,Z,FACTOR
      INTEGER IP,IQ,J,IPREMAX,IERR
      CHARACTER*256 ERRSTR
      COMMON/CONSTS/ZERO,HALF,ONE,TWO,TEN,EPS
	COMMON/IO/NOUT
      DOUBLE PRECISION ZERO,HALF,ONE,TWO,TEN,EPS
...
*     
 ...
 10   CONTINUE
      WRITE (ERRSTR,*) 'ERROR IN IPREMAX: DID NOT FIND MAXIMUM EXPONENT'
      IPREMAX=999
      IERR=1
      RETURN
      STOP
...
C     *                                                              *
C     ****************************************************************
      FUNCTION CGAMMA (ARG,LNPFQ,IERR,ERRSTR)
*     
...
*               WRITE (NOUT,300)
               WRITE (ERRSTR,301) ARGR,ARGI
               IERR=1
               CGAMMA = DCMPLX(999.99D9,999.99D9)
               RETURN
               STOP '010801'
            ELSE
*     
...
*     
 ...
 301  FORMAT (' ARGUMENT (',1P,1D14.7,',',1D14.7,') TOO CLOSE TO A',
     :     ' POLE. ***** ERROR IN SUBROUTINE CGAMMA *****')
 302  FORMAT (' ARGUMENT TO EXPONENTIAL FUNCTION (',1P,1D14.7,
     :     ') OUT OF RANGE. ***** ERROR IN SUBROUTINE CGAMMA *****')
*     
      END
