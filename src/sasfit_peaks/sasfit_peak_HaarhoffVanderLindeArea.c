...
/*  
	if (center*distortion/(width*width) >= log(DBL_MAX)) {
        z=(x-center)/(sqrt(2)*width);
        if (fabs(a)>10*DBL_EPSILON) {
		y = bckgr+area*width/(center*distortion*sqrt(2.*M_PI)) *
			exp(-0.5*gsl_pow_2(z/width)) /
			(
				0. + a
			);
        } else {
			...
			exp(-gsl_pow_2(z)) /
			(
				1./(exp(center*distortion/(width*width)-gsl_pow_2(z))-exp(-gsl_pow_2(z))) 
                -1./(2*sqrt(M_PI)*z)+1./(4*sqrt(M_PI)*gsl_pow_3(z))
			);
        }
	} else if (fabs(center*distortion/width)<1e-6) {
        y=bckgr+area/(width*sqrt(2.*M_PI)) *
			exp(-0.5*gsl_pow_2(z/width)) *
			(
				1 + center*x/gsl_pow_2(width)-center*distortion*gsl_sf_erf(x/(sqrt(2)*width))/(2*gsl_pow_2(width))
			);
    } else if (fabs(a-1) < 100*DBL_EPSILON // && fabs(gsl_sf_expm1(center*distortion/(width*width))+1) < DBL_EPSILON
                                      ) {
        if ((2*center*distortion + gsl_pow_2(-z))/(2*gsl_pow_2(width)) < log(DBL_MAX)/2.
           ) {
             y=bckgr-(area/(center*distortion*(-((exp((2*center*distortion + gsl_pow_2(-z))/(2.*gsl_pow_2(width)))*
              sqrt(2*M_PI))/((-1 + exp((center*distortion)/gsl_pow_2(width)))*width)) + 
         (gsl_pow_5(center) - 10*gsl_pow_3(center)*gsl_pow_2(width) + 15*center*gsl_pow_4(width) + 
            (gsl_pow_4(center) - 6*gsl_pow_2(center)*gsl_pow_2(width) + 3*gsl_pow_4(width))*x + 
            center*(gsl_pow_2(center) - 3*gsl_pow_2(width))*gsl_pow_2(x) + 
            (center - width)*(center + width)*gsl_pow_3(x) + center*gsl_pow_4(x) + gsl_pow_5(x))/gsl_pow_6(x))));
        } else {
            y=bckgr;
        }
	} else {
        if (fabs(distortion)>1e-6) {
		y = bckgr+area*width/(center*distortion*sqrt(2.*M_PI)) *
			exp(-0.5*pow(z/width,2.)) /
			(
				1./(gsl_sf_expm1(center*distortion/(width*width))) + a
			);
        } else {
            b = ((1 - 2*a*(7 + 6*a*(-3 + 2*a)))*gsl_pow_4(center)*gsl_pow_3(distortion))/
                (24.*gsl_pow_8(width)) + ((1 + 6*(-1 + a)*a)*gsl_pow_3(center)*
                gsl_pow_2(distortion))/(6.*gsl_pow_6(width)) + 
                ((1 - 2*a)*gsl_pow_2(center)*distortion)/(2.*gsl_pow_4(width)) + 
                center/gsl_pow_2(width);
            
            y = bckgr+area*width/(center*sqrt(2.*M_PI)) *
                exp(-0.5*pow((x-center)/width,2.)) * b;
        }
	}
    sasfit_out("x%lf y%lf\n",x,y);
	return y;
*/
}
