...
{
    scalar  qR,ql,qA,Psi,l,L,Mr,mr,ms,beta,Sr,Sp,Sm,sinqA_qA,sinql2_ql2;
    ...
    Sp=2*pow(ms,2)*pow(Psi,2)*((NP/(1-sinqA_qA))-(NP/2)-((1-pow(sinqA_qA,NP))/pow(1-sinqA_qA,2))*sinqA_qA);
    Sr=pow(mr,2)*(Mr*(2*L-pow(sinql2_ql2,2))+(2*Mr*pow(beta,2)/(1-sinqA_qA))- 2*pow(beta,2)*(1-pow(sinqA_qA,Mr))/pow(1-sinqA_qA,2));
    Sm=mr*beta*ms*Psi*4*(((NP-1)/(1-sinqA_qA))-((1-pow(sinqA_qA,NP-1))/pow(1-sinqA_qA,2))*sinqA_qA);
