...
scalar sasfit_ff___generalized_gamma_distribution__area_(scalar x, sasfit_param * param)
{
    scalar z;
	...
	z = (x-XI)/SIGMA;
	if (x<=XI) return 0.0;
	if (KAPPA == 0) return A*fabs(ALPHA)*pow(BETA,NU)/gsl_sf_gamma(NU)*pow(z,ALPHA*NU-1)*exp_kappa(KAPPA,-BETA*pow(z,ALPHA),param);
	return A/SIGMA*(1+KAPPA*NU)*pow(2*KAPPA*BETA,NU)*ALPHA*
	        pow(z,ALPHA*NU-1)*exp_kappa(KAPPA,-BETA*pow(z,ALPHA),param)*
	        exp(gsl_sf_lngamma(1./(2*KAPPA)+NU/2)-gsl_sf_lngamma(1./(2*KAPPA)-NU/2)-gsl_sf_lngamma(NU));

}

scalar sasfit_ff___generalized_gamma_distribution__area__f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_ff___generalized_gamma_distribution__area__v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
