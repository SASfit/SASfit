...
scalar tube_model_WE(scalar x, sasfit_param *param) {
	scalar Zmu[3], lambda_mu[3], xi[3], dmu[3], Prod;
	int i;
	lambda_mu[0]=LAMBDA;
	...
	for (i=0;i<3;i++) {
		...
		Prod=Prod
			   -gsl_pow_2(Zmu[i]*lambda_mu[i])*x
		       +gsl_pow_2(Zmu[i]*lambda_mu[i])*xi[i]*(1-exp(-x/(     xi[i])))
               -gsl_pow_2(Zmu[i]             )*xi[i]*(1-exp(-x/(     xi[i]))) ;
	}
	return 2*(1-x)*exp(Prod);
}
scalar sasfit_ff_we__deformed_polym__netw_(scalar q, sasfit_param * param)
{
	...
	return I0*sasfit_integrate(0.0,1.0,&tube_model_WE,param);
}

scalar sasfit_ff_we__deformed_polym__netw__f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_ff_we__deformed_polym__netw__v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
