...
scalar tube_model_HSHring(scalar x, sasfit_param *param) {
	scalar Zmu[3], lambda_mu[3], xi[3], dpsi,xi_psi, Prod, ij;
	int i;
	ij=x*(1-x);
	lambda_mu[0]=LAMBDA;
	...
	for (i=0;i<3;i++) {
		Prod=Prod
			   -gsl_pow_2(Zmu[i]*lambda_mu[i])*ij/GAMA
		       +gsl_pow_2(Zmu[i]*lambda_mu[i])*xi_psi*(1-exp(-ij/(GAMA*xi_psi)))
               -gsl_pow_2(Zmu[i]             )*xi_psi*(1-exp(-ij/(     xi_psi))) ;
	}
	return 2*(1-x)*exp(Prod);
}

scalar sasfit_ff_hsh__deformed_ringpolym__netw_(scalar q, sasfit_param * param)
{
	...
	return I0*sasfit_integrate(0.0,1.0,&tube_model_HSHring,param);
}

scalar sasfit_ff_hsh__deformed_ringpolym__netw__f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_ff_hsh__deformed_ringpolym__netw__v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
