...
scalar sasfit_peak_haarhoff_vander_linde_area(scalar x, sasfit_param * param)
{
	scalar y,k,z, cond0;
...
//sasfit_out("0 HLV(%lf)=%lf\n",x,y);
    ...
    if (k<= log(DBL_MAX)/1.1 && k >0) {
        y=BACKGR+AREA*exp(-z*z)/(k*sqrt(2*M_PI)*WIDTH*(1/(exp(k)-1)+derfc(-z)/2));
//sasfit_out("1 HLV(%lf)=%lf\n",x,y);
    ...
    if (k>= -log(DBL_MAX)/1.1 ) {
        ...
        if (fabs(cond0) > 0.) {
            ...
            y=BACKGR+AREA*exp(-z*z)/(k*sqrt(2*M_PI)*WIDTH)/cond0;
//sasfit_out("1a HLV(%lf)=%lf cond0=%lg k=%lg, z>%lg, erfc(-z)=%lg\n",x,y,cond0, k,z,derfc(-z));
            return y;
        } 
        if (k+z*z <= log(DBL_MAX)/1.1) {
            y=BACKGR+AREA/(exp(k+z*z)*sqrt(2*M_PI)/(exp(k)-1) - (3-2*z*z+4*gsl_pow_4(z))/(4*sqrt(2)*gsl_pow_5(z)))/(k*WIDTH);
...
scalar sasfit_peak_haarhoff_vander_linde_area_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
