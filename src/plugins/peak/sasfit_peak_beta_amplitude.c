...
scalar sasfit_peak_beta_help(scalar x, sasfit_param * param)
{
	scalar z, xmin, xmax;
...
scalar sasfit_peak_beta_mode(sasfit_param * param)
{
	scalar xmin,xmax;
	if (XMIN>XMAX) {
		xmin = XMAX;
...
scalar sasfit_peak_beta_amplitude(scalar x, sasfit_param * param)
{
	scalar mode, a0;
	...
	mode = sasfit_peak_beta_mode(param);
	a0 = AMPL/sasfit_peak_beta_help(mode,param);
	return BCKGR+a0*sasfit_peak_beta_help(x,param);
}

scalar sasfit_peak_beta_amplitude_f(scalar q, sasfit_param * param)
{
...
scalar sasfit_peak_beta_amplitude_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
