/*
 * src/sasfit_peaks/sasfit_peak_ErrorArea.c
 *
...
scalar sasfit_peak_error_area(scalar x, sasfit_param * param)
{
	scalar z,a0;
	...
	a0 = AREA/(
				pow(WIDTH,0.5*SHAPE) * 
				pow(2.,.5*SHAPE+1.) *
				gsl_sf_gamma(.5*SHAPE+1.)
			);
	z = .5*pow(fabs(x-CENTER),2./SHAPE)/WIDTH;
	return BACKGR+a0*exp(-z);
}

scalar sasfit_peak_error_area_f(scalar q, sasfit_param * param)
{
...
scalar sasfit_peak_error_area_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
