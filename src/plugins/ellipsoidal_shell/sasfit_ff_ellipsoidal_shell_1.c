...
scalar sasfit_ff_ellip_shell_1_core(sasfit_param * param) {
	A = R_MAJOR;
	B = R_MINOR;
	TNU = T;
...
scalar sasfit_ff_ellip_shell_1_core_NU(scalar x, sasfit_param * param)
{
	...
	return sasfit_ff_ellip_shell_1_core(param);
}


scalar sasfit_ff_ellip_shell_1_core_MU(scalar x, sasfit_param * param)
{
	scalar res;
	...
	res = sasfit_integrate(NUMIN, NUMAX, sasfit_ff_ellip_shell_1_core_NU, param);

	return res;
}

scalar sasfit_ff_ellipsoidal_shell_1(scalar q, sasfit_param * param)
{
	...
	cparam.ndim=ndim;
	cparam.func = &sasfit_ff_ellip_shell_1_core;
...
//		    sasfit_out("ise default sasfit_integrate routine\n");
            sum=sasfit_integrate(0.0, 1.0, sasfit_ff_ellip_shell_1_core_MU, param);
            break;
            }
    }
	return sum;
...
scalar sasfit_ff_ellipsoidal_shell_1_f(scalar q, sasfit_param * param)
{
	scalar *aw, res,err,sum;
    scalar cubxmin[2], cubxmax[2], fval[1], ferr[1];
    size_t neval;
...
scalar sasfit_ff_ellipsoidal_shell_1_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
