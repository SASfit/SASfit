...
scalar sasfit_ff_ellip_shell_rp_t_core(sasfit_param * param) {
	A = R_PRINCIPLE;
	B = R_EQUATORIAL;
	TNU = T;
...
scalar sasfit_ff_ellip_shell_rp_t_core_NU(scalar x, sasfit_param * param)
{
	...
	return sasfit_ff_ellip_shell_rp_t_core(param);
}


scalar sasfit_ff_ellip_shell_rp_t_core_MU(scalar x, sasfit_param * param)
{
	scalar res;
	...
	res = sasfit_integrate(NUMIN, NUMAX, sasfit_ff_ellip_shell_rp_t_core_NU, param);

	return res;
}

scalar sasfit_ff_ellipsoidal_shell_rp_t(scalar q, sasfit_param * param)
{
	...
	cparam.ndim=ndim;
	cparam.func = &sasfit_ff_ellip_shell_rp_t_core;
...
//		    sasfit_out("ise default sasfit_integrate routine\n");
            sum=sasfit_integrate(0.0, 1.0, sasfit_ff_ellip_shell_rp_t_core_MU, param);
            break;
            }
    }
	return sum;
...
scalar sasfit_ff_ellipsoidal_shell_rp_t_f(scalar q, sasfit_param * param)
{
	scalar *aw, res,err,sum;
    scalar cubxmin[2], cubxmax[2], fval[1], ferr[1];
    size_t neval;
...
scalar sasfit_ff_ellipsoidal_shell_rp_t_v(scalar q, sasfit_param * param, int dist)
{
	...
	V = (T3+RP*T2)*nu3+(2*RE*T2+2*RE*RP*TT)*nu2+(RE2*TT+RE2*RP)*nu1;
