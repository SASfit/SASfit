...
scalar sasfit_ff_ellip_shell_0_core_NU(scalar x, sasfit_param * param)
{
	...
	return sasfit_ff_ellip_shell_0_core(param);
}


scalar sasfit_ff_ellip_shell_0_core_MU(scalar x, sasfit_param * param)
{
	scalar res;
	...
	res = sasfit_integrate(NUMIN, NUMAX, sasfit_ff_ellip_shell_0_core_NU, param);

	return res;
}

scalar sasfit_ff_ellipsoidal_shell_0(scalar q, sasfit_param * param)
{
    int ierr,ndim;
	scalar res,err;
	...
	cparam.ndim=ndim;
	cparam.func = &sasfit_ff_ellip_shell_0_core;

    ierr = sasfit_cubature(ndim,cubxmin,cubxmax,&ellip_shell_cubature,&cparam,sasfit_eps_get_nriq(), &res, &err);
    return res;
}

scalar sasfit_ff_ellipsoidal_shell_0_f(scalar q, sasfit_param * param)
{
    int ierr,ndim;
	scalar res,err;
    scalar cubxmin[2], cubxmax[2];
