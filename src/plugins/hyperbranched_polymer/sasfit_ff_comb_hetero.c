...
scalar sasfit_ff_comb_hetero(scalar q, sasfit_param * param)
{
    scalar X,X2,X4,Comb;
	...
	Comb = 2*I0/X4/(1+gsl_pow_2(1-K)/F) *
            (
             X2+expm1(-X2*K)
               - expm1(-X2*(1-K)/F)*(F+2*expm1(-X2*K)/(X2*K/F))
               +gsl_pow_2(expm1(-X2*(1-K)/F)) * (X2*K+expm1(-X2*K))/gsl_pow_2(X2*K/F)
             );
    if (gsl_isnan(Comb)) {
        return 2*I0/X4/(1+gsl_pow_2(1-K)/F);
    } else {
        return Comb;
    }
}

scalar sasfit_ff_comb_hetero_f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_ff_comb_hetero_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
