...
scalar egu_w_l_profile_PcsCyl(scalar x, sasfit_param *param) {
	scalar u,fsp,qmod,y;
	qmod = Q;
...
scalar sasfit_ff_egu_w_l_profile_PcsCyl(scalar q, sasfit_param * param)
{
	...
	return gsl_pow_2(sasfit_ff_egu_w_l_profile_PcsCyl_f(q,param));
}

scalar sasfit_ff_egu_w_l_profile_PcsCyl_f(scalar q, sasfit_param * param)
{
    scalar ystart, yend;
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
	...
	return sasfit_integrate(0,BU,&egu_w_l_profile_PcsCyl,param)*gsl_pow_2(RSCALE);
}

scalar sasfit_ff_egu_w_l_profile_PcsCyl_v(scalar u, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
    return 0.0;
}
