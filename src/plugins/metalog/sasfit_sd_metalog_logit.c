...
scalar sasfit_sd_metalog_logit(scalar x, sasfit_param * param)
{
	...
	if (gsl_finite(pow(x,-ALPHA))) {
        return sasfit_sd_metalog_logit_v(x, param,DISTRIBUTION_PROBABILITY)*pow(x,-ALPHA);
    } else {
        return 0;
    }
}
...
scalar sasfit_sd_metalog_logit_v(scalar u, sasfit_param * param, int dist)
{
	...
	switch (dist) {
        ...
        case DISTRIBUTION_PROBABILITY:
                ...
                px = 1.0/sasfit_sd_metalog_logit_v(y,param,DISTRIBUTION_QUANTILE_DENS);
                N = p0tmp;
                return N*px;
        ...
        case DISTRIBUTION_MEDIAN:
                return sasfit_sd_metalog_logit_v(0.5,param,DISTRIBUTION_QUANTILE);
                break;
        case DISTRIBUTION_SKEWNESS:
                LQ = sasfit_sd_metalog_logit_v(0.25,param,DISTRIBUTION_QUANTILE);
                UQ = sasfit_sd_metalog_logit_v(0.75,param,DISTRIBUTION_QUANTILE);
                M  = sasfit_sd_metalog_logit_v(0.5,param,DISTRIBUTION_QUANTILE);
                IQR = UQ - LQ;
        ...
        case DISTRIBUTION_EXCESS_KURTOSIS:
                LQ = sasfit_sd_metalog_logit_v(0.25,param,DISTRIBUTION_QUANTILE);
                UQ = sasfit_sd_metalog_logit_v(0.75,param,DISTRIBUTION_QUANTILE);
                K  = sasfit_sd_metalog_logit_v(7./8.,param,DISTRIBUTION_QUANTILE);
                K  = K+sasfit_sd_metalog_logit_v(3./8.,param,DISTRIBUTION_QUANTILE);
                K  = K-sasfit_sd_metalog_logit_v(5./8.,param,DISTRIBUTION_QUANTILE);
                K  = K-sasfit_sd_metalog_logit_v(1./8.,param,DISTRIBUTION_QUANTILE);
                IQR = UQ - LQ;
                return K/IQR;
                break;
