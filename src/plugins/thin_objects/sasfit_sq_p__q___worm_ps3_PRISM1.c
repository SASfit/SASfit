...
scalar sasfit_sq_p__q___worm_ps3_PRISM1(scalar q, sasfit_param * param)
{
...
// here method 3 of the above paper is implemented.
	...
	cq=gsl_sf_bessel_j0(q*RC)*exp(-gsl_pow_2(q*sigma));
	return Swc/(1+BETA*cq*Swc/S0);
}

scalar sasfit_sq_p__q___worm_ps3_PRISM1_f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_sq_p__q___worm_ps3_PRISM1_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
