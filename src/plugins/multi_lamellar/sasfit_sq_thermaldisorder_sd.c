...
scalar thermaldisorder_core(scalar x, void * pam)
{
    sasfit_param * param;
	param = (sasfit_param *) pam;

    N = NSIGN*x;
    return 1./(SIGMA_SD*sqrt(2*M_PI))*exp(-gsl_pow_2((x-NMEAN)/SIGMA_SD)/2.)*(sasfit_sq_thermaldisorder(Q,param)-NU);
}

scalar sasfit_sq_thermaldisorder_sd(scalar q, sasfit_param * param)
{
...
//	SASFIT_CHECK_COND1((N < 0.0), param, "N(%lg) < 0",N); // modify condition to your needs
    ...
    sasfit_intdeo(&thermaldisorder_core,NMEAN-2*SIGMA_SD,Q*D, aw, &res, &err, param);
    free(aw);
    return res+NU;
//    return sasfit_integrate(NMEAN-2*SIGMA_SD,NMEAN+2*SIGMA_SD,&thermaldisorder_core,param)+NU;
}

scalar sasfit_sq_thermaldisorder_sd_f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_sq_thermaldisorder_sd_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
