...
scalar sasfit_sq_thermaldisorder_sum(scalar q, sasfit_param * param)
{
...
//	SASFIT_CHECK_COND1((N < 0.0), param, "N(%lg) < 0",N); // modify condition to your needs
    ...
    for (n=lround(NN-2*SIGMA_SD);n<=lround(NN+2*SIGMA_SD);n++) {
        SQ1=n;SQ2=n+1;
        for (k=1;k<n;k++) {
            SQ1 = SQ1+2*(n  -k)*cos(k*qd)*exp(-qD2/2.);
            SQ2 = SQ2+2*(n+1-k)*cos(k*qd)*exp(-qD2/2.);
        }
        SQ2 = SQ2+2*cos(n*qd)*exp(-qD2/2.);
...
scalar sasfit_sq_thermaldisorder_sum_f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_sq_thermaldisorder_sum_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
