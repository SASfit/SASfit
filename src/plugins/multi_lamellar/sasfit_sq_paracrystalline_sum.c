...
scalar sasfit_sq_paracrystalline_sum(scalar q, sasfit_param * param)
{
    scalar q2,s2, SQ1, SQ2,SQ1sum,SQ2sum,qd,qD2,w;
    long NN,j,k,n;
...
//	SASFIT_CHECK_COND1((DELTA < 0.0), param, "Delta(%lg) < 0",DELTA); // modify condition to your needs
    ...
    for (n=lround(NN-2*SIGMA_SD);n<=lround(NN+2*SIGMA_SD);n++) {
        SQ1=n;SQ2=n+1;
        for (k=1;k<n;k++) {
            SQ1 = SQ1+2*(n  -k)*cos(k*qd)*exp(-k*qD2/2.);
            SQ2 = SQ2+2*(n+1-k)*cos(k*qd)*exp(-k*qD2/2.);
        }
        SQ2 = SQ2+2*cos(n*qd)*exp(-n*qD2/2.);
...
scalar sasfit_sq_paracrystalline_sum_f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_sq_paracrystalline_sum_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
