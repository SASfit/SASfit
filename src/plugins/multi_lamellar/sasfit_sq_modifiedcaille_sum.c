...
scalar sasfit_sq_modifiedcaille_sum(scalar q, sasfit_param * param)
{
    scalar SQ1, SQ2,SQ1sum,SQ2sum,qd,qD2,w,tmp;
    long NN,j,k,n;
    ...
    for (n=lround(NN-2*SIGMA_SD);n<=lround(NN+2*SIGMA_SD);n++) {
        ...
        SQ2 = SQ2+2*cos(n*qd)*exp(-pow(qd*0.5/M_PI,2.0)*ETA*(M_EULER+log(M_PI*n)));
...
scalar sasfit_sq_modifiedcaille_sum_f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_sq_modifiedcaille_sum_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
