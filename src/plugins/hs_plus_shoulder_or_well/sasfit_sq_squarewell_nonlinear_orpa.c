...
scalar sasfit_sq_square_well_nonlin_orpa(scalar q, sasfit_param * param)
{
    scalar S;
	static scalar FP_old=-1.0;
	static scalar LAMBDA_old=-1.0;
	static scalar EPSI_KT_old=-1.0;
	static sw_nonlinear_orpa_t *orpa = NULL;
	int solve_opra;
	...
	if ((FP_old!=FP) || (LAMBDA_old!=LAMBDA) || (EPSI_KT_old!=EPSI_KT))
	{
	    if (orpa!=NULL) sw_nonlinear_orpa_free(orpa);
	    orpa = sw_nonlinear_orpa_create(FP, LAMBDA, 60.0, 6000);
	    solve_opra=sw_nonlinear_orpa_solve(orpa, EPSI_KT, 15, 0.05);
	    FP_old=FP;
	    LAMBDA_old=LAMBDA;
	    EPSI_KT_old=EPSI_KT;
	}
    ...
    if (solve_opra) {
        S = sw_nonlinear_orpa_S(orpa, q * 2*RHS);
    } else {
        sasfit_err("did not converge\n");
    }

	return S;
}

scalar sasfit_sq_square_well_nonlin_orpa_f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_sq_square_well_nonlin_orpa_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
