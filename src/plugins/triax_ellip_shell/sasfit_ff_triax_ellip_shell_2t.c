...
scalar sasfit_ff_triax_ellip_shell_2t_core(sasfit_param * param)
{
	scalar u_c, u_sh;
	scalar f_c, f_sh;
...
scalar sasfit_ff_triax_ellip_shell_2t_core_NU(scalar x, sasfit_param * param)
{
	...
	return sasfit_ff_triax_ellip_shell_2t_core(param);
}

scalar sasfit_ff_triax_ellip_shell_2t_core_y(scalar y, sasfit_param * param)
{
	scalar res;
	...
	if (SIGMA == 0) {
		NU = 1;
		LNDISTR = 1;
		res = sasfit_ff_triax_ellip_shell_2t_core(param);
	} else {
		res = sasfit_integrate(NUMIN, NUMAX, sasfit_ff_triax_ellip_shell_2t_core_NU, param);
	}

	return res;
}

scalar sasfit_ff_triax_ellip_shell_2t_core_x(scalar x, sasfit_param * param)
{
	scalar res;
	...
	res = sasfit_integrate(0.0, 1.0, sasfit_ff_triax_ellip_shell_2t_core_y, param);

	return res;
}

scalar sasfit_ff_triax_ellip_shell_2t(scalar q, sasfit_param * param)
{
	...
	cparam.ndim=ndim;
	cparam.func = &sasfit_ff_triax_ellip_shell_2t_core;
...
//		    sasfit_out("ise default sasfit_integrate routine\n");
            sum=sasfit_integrate(0.0, 1.0, sasfit_ff_triax_ellip_shell_2t_core_x, param);
            break;
            }
    }
	return sum;
...
scalar sasfit_ff_triax_ellip_shell_2t_f(scalar q, sasfit_param * param)
{
	scalar *aw, res,err,sum;
    scalar cubxmin[3], cubxmax[3], fval[1], ferr[1];
    size_t neval;
...
scalar sasfit_ff_triax_ellip_shell_2t_v(scalar x, sasfit_param * param, int dist)
{
	...
	V = (T*T*T+B*T*T+A*T*T+A*B*T)*nu3+(C*T*T+B*C*T+A*C*T+A*B*C)*nu2;
	V=V*4./3.*M_PI;
	return V;
