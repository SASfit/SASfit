...
#define RG	param->p[6]
#define F_PEG_I	fabs(param->p[7])
#define F_PEG_O	fabs(param->p[8])
...
scalar vesicle_pegulated_laplace(const double* v, size_t n, void* p)
{
    scalar Phi, ni, no, N, Ri, Ro, Vtail,Vhead,HiL, HiU,HoL, HoU,HoGuess, HiGuess, VheadL,VheadU, Vho,Vhi,tmp, Ichain,Iii,Ioo,Iio,Ivi,Ivo;
    sasfit_param * param;
    double qrc[9],value;
    char sBuffer[256];
    int ierr,i, status;
...
// initial guess of HI
    HI = TINT*V_HEAD/V_TAIL*5;
    HiU=HI;
    if (RIH<0 || !gsl_finite(Vbilayer(RIH,RIT,SIGMA_IH,SIGMA_IH,param))) {
        HI = TINT*V_HEAD/V_TAIL*2;
        HiU=HI;
    }
    if (RIH<0) {
//        sasfit_out("Too small radius: Q:%lg, R:%lg, HI:%lg \n",Q,RINT,HI);
        return 0;
    }
    VheadU = Vbilayer(RIH,RIT,SIGMA_IH,SIGMA_IH,param);
    HI = 0;
    HiL = HI;
    VheadL = Vbilayer(RIH,RIT,SIGMA_IH,SIGMA_IH,param);
    status = 0;
    for ( ; ; )
    {
        zero_rc(HiL,HiU,DBL_EPSILON,&HiGuess,&status,value);
        if ( status < 0 )
        {
 //           sasfit_err ( "  zero_rc() returned an error flag!\n" );
            if (status == -1) {
//                sasfit_out(".");
//                sasfit_out("zero_rc error (no change of sign): Q:%lg, R:%lg, HI:%lg, status:%d \n",Q,RINT,HI,status);
            } else {
                sasfit_out("zero_rc error: Q:%lg, R:%lg, HI:%lg, status:%d \n",Q,RINT,HI,status);
            }
            return 0;
        }
        HI = HiGuess;
        value = Vbilayer(RIH,RIT,SIGMA_IH,SIGMA_IH,param)-Vhi;

//        sasfit_out ( "  %8d  %24.16e  %10e\n", status, HiGuess, value );
...
// initial guess of HO
    HO = TINT*V_HEAD/V_TAIL*10;
    HoU=HO;
//    VheadU = Vbilayer(ROT,ROH,SIGMA_OH,SIGMA_OH,param);
    if (!gsl_finite(Vbilayer(ROH,ROT,SIGMA_OH,SIGMA_OH,param))) {
        HO = TINT*V_HEAD/V_TAIL*5;
        HoU=HO;
        if (!gsl_finite(Vbilayer(ROH,ROT,SIGMA_OH,SIGMA_OH,param))) {
            HO = TINT*V_HEAD/V_TAIL*2;
            HoU=HO;
            if (!gsl_finite(Vbilayer(ROH,ROT,SIGMA_OH,SIGMA_OH,param))) {
                HO = TINT*V_HEAD/V_TAIL;
                HoU=HO;
            }
        }
    }
    HO = 0;
    HoL = HO;
//    VheadL = Vbilayer(ROT,ROH,SIGMA_OH,SIGMA_OH,param);
    status = 0;
    for ( ; ; )
    {
        zero_rc(HoL,HoU,DBL_EPSILON,&HoGuess,&status,value);
        if ( status < 0 )
        {
 //           sasfit_err ( "  zero_rc() returned an error flag! %d\n",status );
            return 0;
        }
        HO = HoGuess;
        value = Vbilayer(ROT,ROH,SIGMA_OH,SIGMA_OH,param)-Vho;
...
/*
    ...
    } while (fabs(xerr/HO)>1e-6&RTH[x_, Rc_, sigmaC_, Rh_, sigmah_] := (1 - RT[x, Rc, sigmaC])*
  RT[x, Rh, sigmah]&ierr==0);
    if (ierr>0) {
        sasfit_err("%s\n",sBuffer);
        return 0;
    }
...
scalar vesicle_pegulated_laplace_kernel(const double* x, size_t n, void* p)
{
    sasfit_param * param;
    ...
    return vesicle_pegulated_laplace(x,n,p)*LNdistrT*LNdistrR;
}

scalar vesicle_pegulated_laplace_kernel_dQR(scalar x, sasfit_param * param)
{
    scalar v[2];
	scalar LNdistr;
	sasfit_param subParam;
    ...
    v[0]=x;
    v[1]=T;
    return vesicle_pegulated_laplace(v,2,param)*LNdistr;
}

scalar vesicle_pegulated_laplace_kernel_dQT(scalar x, sasfit_param * param)
{
    scalar v[2];
	scalar LNdistr;
	sasfit_param subParam;
...
scalar sasfit_ff_vesicle_peg_laplace(scalar q, sasfit_param * param)
{
    int lenaw = 4000;
    scalar *aw;
    scalar res, err;
	...
	if (RStart==REnd && TStart==TEnd) return vesicle_pegulated_laplace(v,2,param);
    if (RStart==REnd ) return sasfit_integrate(TStart,TEnd,vesicle_pegulated_laplace_kernel_dQT,param);
    if (TStart==TEnd) return sasfit_integrate(RStart,REnd,vesicle_pegulated_laplace_kernel_dQR,param);
	...
	sasfit_cubature(2,
			intstart,
			intend,
			&vesicle_pegulated_laplace_kernel,
			param,
			sasfit_eps_get_nriq(),
			&res2D,
			&err2D);
...
scalar sasfit_ff_vesicle_peg_laplace_f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_ff_vesicle_peg_laplace_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
