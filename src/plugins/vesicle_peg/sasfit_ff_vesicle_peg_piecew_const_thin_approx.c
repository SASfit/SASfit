...
scalar vesicle_pegulated_pcs(const double* v, size_t n, void* p)
{
...
scalar vesicle_pegulated_pcs_kernel(const double* x, size_t n, void* p)
{
    sasfit_param * param;
    ...
    return vesicle_pegulated_pcs(x,n,p)*LNdistrT*LNdistrR;
}


scalar sasfit_ff_vesicle_peg_piecew_const_thin_approx(scalar q, sasfit_param * param)
{
    int lenaw = 4000;
    scalar *aw;
    scalar res, err;
	...
	if (RStart==REnd && TStart==TEnd) return vesicle_pegulated_pcs(v,2,param);
    if (RStart==REnd ) return vesicle_pegulated_pcs(v,2,param);;
    if (TStart==TEnd) return vesicle_pegulated_pcs(v,2,param);;
...
scalar sasfit_ff_vesicle_peg_piecew_const_thin_approx_f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_ff_vesicle_peg_piecew_const_thin_approx_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
