...
scalar sasfit_ff_ellcylsh_sd_worm_ps1(scalar q, sasfit_param * param)
{
	scalar Pcs,Pprime;
	...
	subParam.p[1] = LL;
	subParam.p[2] = LEXV;
	Pprime = sasfit_sq_p__q___worm_ps1_(q,&subParam);

	return Pcs*Pprime;
}

scalar sasfit_ff_ellcylsh_sd_worm_ps1_f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_ff_ellcylsh_sd_worm_ps1_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
