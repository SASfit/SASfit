...
scalar crJ0(scalar rp,sasfit_param * param) {
    scalar cr,a,p,sigma;
...
scalar sasfit_sq__d_hard_disks__guo_(scalar q, sasfit_param * param)
{
    scalar Cq;
	...
	Q=q;
    Cq=sasfit_integrate(0,1,&crJ0,param);
    return 1/(1-ETA/(M_PI*R*R)*Cq);
}

scalar sasfit_sq__d_hard_disks__guo__f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_sq__d_hard_disks__guo__v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
