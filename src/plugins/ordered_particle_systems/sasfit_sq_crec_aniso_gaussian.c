...
scalar sasfit_sq_crec_aniso_gaussian(scalar x, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid

	SASFIT_CHECK_COND1((PAD < 0.0), param, "a(%lg) < 0",PAD); // modify condition to your needs
	SASFIT_CHECK_COND1((PBD < 0.0), param, "a(%lg) < 0",PBD); // modify condition to your need
...
scalar sasfit_sq_crec_aniso_gaussian_f(scalar q, sasfit_param * param)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
...
scalar sasfit_sq_crec_aniso_gaussian_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
