...
scalar sasfit_ff_cone_opo(scalar q, sasfit_param * param)
{
    ...
    opod.Q[2] = 0;
    opo_setQhat(&opod);
	return gsl_pow_2((ETA_P-ETA_M) *opod.detDinv*opo_Fcone(&opod));
}
...
scalar sasfit_ff_cone_opo_f(scalar q, sasfit_param * param)
{
 ...
 //   opod.Rotation.convention =Z1_X2_Y3;
    ...
    opod.Q[2] = 0;
    opo_setQhat(&opod);
	return (ETA_P-ETA_M) *opod.detDinv*opo_Fcone(&opod);
}
...
scalar sasfit_ff_cone_opo_v(scalar q, sasfit_param * param, int dist)
{
	...
	// insert your code here
	return 2*tan(TILT)/sqrt(3)*(1-gsl_pow_3(1-H_R*opo_Cot(TILT)))*opod.detDinv;
}
