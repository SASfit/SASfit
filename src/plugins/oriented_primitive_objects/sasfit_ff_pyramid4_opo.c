...
scalar sasfit_ff_pyramid4_opo(scalar q, sasfit_param * param)
{
    scalar psi;
	opo_data opod;
    ...
    opod.Q[2] = 0;
    opo_setQhat(&opod);
	return gsl_pow_2((ETA_P-ETA_M) *opod.detDinv*opo_Fpyramid4(&opod));
}

scalar sasfit_ff_pyramid4_opo_f(scalar q, sasfit_param * param)
{
    scalar psi;
	opo_data opod;
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
 ...
 //   opod.Rotation.convention =Z1_X2_Y3;
    ...
    opod.Q[2] = 0;
    opo_setQhat(&opod);
	return (ETA_P-ETA_M) *opod.detDinv*opo_Fpyramid4(&opod);
}

scalar sasfit_ff_pyramid4_opo_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
