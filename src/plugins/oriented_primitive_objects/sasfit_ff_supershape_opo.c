...
scalar sasfit_ff_supershape_opo(scalar q, sasfit_param * param)
{
	opo_data opod;
...
scalar sasfit_ff_supershape_opo_f(scalar q, sasfit_param * param)
{
	opo_data opod;
	scalar *aw, res,err,sum;
    scalar cubxmin[1], cubxmax[1], fval[1], ferr[1];
    ...
    QQY = opod.Qhat[1];
    QQZ = opod.Qhat[2];
	return (ETA_P-ETA_M)*opod.detDinv*opo_Fsupershape(param);
}

scalar sasfit_ff_supershape_opo_v(scalar q, sasfit_param * param, int dist)
{
	SASFIT_ASSERT_PTR(param); // assert pointer param is valid
