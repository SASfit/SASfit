...
/*#                                                                       #*/
/*#########################################################################*/
scalar Rav(q,q_av,sigma_t)
scalar q,q_av,sigma_t;
{
	scalar ax = 0.0, result = 0.0;
	if (sigma_t == 0.0) {
		if (q == q_av) {
			result = 1.0;
		} else {
        ...
        result = q/(sigma_t*sigma_t)
                * exp(-0.5*gsl_pow_2((q-q_av)/sigma_t))
                * gsl_sf_bessel_I0_scaled(ax);
	}
	return result;
}
...
float q_av;  /* average Q value in 1/nm									*/
{
	...
	sigma_D1 = k_av*cos(theta_av)*pow(cos(theta2_av),2.0)*D/l  / sqrt(12.0);
	sigma_D2 = k_av*                  cos(theta2_av)     *D/l  / sqrt(12.0);
	sigma_av = k_av*cos(theta_av)*pow(cos(theta2_av),2.0)*Dd/l / sqrt(12.0);
