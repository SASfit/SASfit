...
void DoubleCumulant(double,double*,double*, double*, bool*);
...
//  double argexp;
  ...
  for (i = 0; i < n; i++)
    {
//      argexp = -2.0*Par[1]*t[i]+Par[2]*t[i]*t[i]-Par[3]*t[i]*t[i]*t[i]/3.0;
//	  Yi = Par[0] * exp (argexp);

	if ( strcmp(DLSmodel,"cumulant analysis") == 0 ) {
		Cumulant(t[i],Par,&Yi, dydpar, &error);
	}
	if ( strcmp(DLSmodel,"double decay cumulant analysis") == 0 ) {
		DoubleCumulant(t[i],Par,&Yi, dydpar, &error);
	}
...
void Cumulant(double x,double *par,double *y, double *dydpar, bool *error)
{
	double argexp;
	argexp = -2.0*par[1]*x
		     +    par[2]*x*x
...
void DoubleCumulant(double x,double *par,double *y, double *dydpar, bool *error)
{
...
bool *error;
{
	if ( strcmp(DLSParData.DLSmodel,"cumulant analysis") == 0 ) {
		Cumulant(x,DLSParData.Par,yth,dydpar,error);
	}
	if ( strcmp(DLSParData.DLSmodel,"double decay cumulant analysis") == 0 ) {
		DoubleCumulant(x,DLSParData.Par,yth,dydpar,error);
	}
	if ( strcmp(DLSParData.DLSmodel,"double stretched exponential") == 0 ) {
