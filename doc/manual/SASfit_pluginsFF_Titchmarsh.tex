...
\end{align}
The parameter $\gamma$ is not essential; its contribution can be incorporated by appropriately correcting the values of $\alpha$ and $\beta$. However, $\alpha$ was introduced to describe how the volume of the objects to which small-angle scattering is sensitive scales with the parameter $a$. For a homogeneous cylinder with random orientation and a radius within the range of the SAS experiments, the object dimension is $d=3$. For an oriented cylinder measured perpendicular to its axis, the signal probes only the cross-section, and $d=2$. If $a$ is the smaller or larger dimension of a very anisotropic object, such as a thin local cylindrical object or a thin local planar object, the volume scales with $V_0a^\alpha l^\gamma$, where $l\gg a$ or $l\ll a$ describes the larger or smaller dimension of the object. The overall dimension of the object is $1\leq d=\alpha+\gamma\leq3$ and should lie between 1 and 3. Furthermore, the overall dimension $d$ of the object must be larger than $\alpha$, i.e., $2\geq\gamma\geq0$. Because only the dimension $a$ is probed when $l\gg a$, the larger dimension $l$ leads to a power-law behaviour at small $q$-values with $s=\gamma$. This additional constraint means that the lesser generalized form factor has three input parameters, either $(d,s,p)$ or $(d,\alpha,p)$.
