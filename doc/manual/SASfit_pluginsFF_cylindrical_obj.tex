...
\begin{subequations}
\begin{align}
\label{eq:CylHEXparacrystal}
  I_\mathrm{cyl}(Q) &= \frac{1}{2\pi}\int_0^\infty I_\perp(Q,\psi) \mathrm{d}\psi\\
  ...
  S(Q) &= Z_1 Z_2  \\
  Z_k &= \frac{1-\abs{F_k}^2}{1-2\abs{F_k}\cos\left(\mathbf{Q}\mathbf{a}_k\right) + \abs{F_k}^2} \\
...
~\\
\subsubsection{Scattering perpendicular to hexagonal packed cylinder} ~\\
...
~\\
\subsubsection{Scattering of randomly oriented bundles of hexagonal packed cylinder} ~\\
