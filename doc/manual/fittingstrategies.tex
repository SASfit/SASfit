...
Within \texttt{SASfit}, a configurable one-dimensional integration routine and a multidimensional routine with a common interface for the different algorithms are supplied. Depending on the mathematical behaviour of the integrand, the different algorithms can differ considerably in efficiency. For multidimensional quadrature algorithms, the same strategy is currently used for all dimensions. The routines \texttt{sasfit\_integrate()} and \texttt{sasfit\_cubature()} have the same configuration options. For multidimensional integrations, either specialized multidimensional algorithms or nested calls to one-dimensional integration routines are used. \\
