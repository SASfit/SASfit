...
&\phantom{=} - e_{az}\, e_{by}\, e_{cx}
   - e_{ay}\, e_{bx}\, e_{cz}
   - e_{ax}\, e_{bz}\, e_{cy}) \label{eq:detDm1}
\end{align}
\begin{align}
...
\subsection{perfect and random oriented carved ellipsoid} ~\\

\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.8\textwidth]{../images/form_factor/Ellipsoid/carved_triaxial_ellipsoid_1.png}
\end{center}
\caption{sketch of a carved triaxial ellipsoidal shell according to \cite{Li2019}}
\label{fig:carved_tri_ell_sh}
\end{figure}

\begin{align}\label{eq:Fcarved_tri_ell}
  F(q,) &= 1 \\
  E &= \\
  H_1 &= \\
  H_2 &=
...
F_{\mathrm{C,rnd}}(Q) &= \left\langle F_\mathrm{C}(\B{Q})\right\rangle_\B{Q} = \int_0^\pi\mathrm{d}\theta \int_0^{2\pi}\mathrm{d}\phi \; F_\mathrm{C}\left(Q\begin{pmatrix}
                                       ...
                                       \sin(\phi)\sin(\theta) \\
                                       \cos(\theta)
                                     \end{pmatrix}\right) \label{eq:opo_rnd_cylF}\\
P_{\mathrm{C,rnd}}(Q) &= \left\langle F_\mathrm{C}^2(\B{Q})\right\rangle_\B{Q} = \int_0^\pi\mathrm{d}\theta \int_0^{2\pi}\mathrm{d}\phi \; F_\mathrm{C}^2\left(Q\begin{pmatrix}
                                       ...
                                       \sin(\phi)\sin(\theta) \\
                                       \cos(\theta)
                                     \end{pmatrix}\right) \label{eq:opo_rnd_cylF2}
\end{align}
~\\
...
The basic formula for the form factor of a blunted cone has been taken from \cite{Renaud2009}. The difference from that version is that the affine transformation from above is used to scale the size of the cone. This is also the reason why the ratio of the height and radius $H_R=\frac{H}{R}$ is used as an input parameter instead of $H$ and $R$ separately; i.e., a unit cone is assumed to have a radius of $R=1$. The overall size is then scaled with the input parameters $a$, $b$ and $c$. The shape does not have inversion symmetry and is therefore a complex function. The phase factor is taken so that the base plane lies in the $xy$-plane according to Fig.\ \ref{fig:opo_cone}. The form factor of the cone according to \cite{Renaud2009} is given as
...
\item[\texttt{dummy}] not used
\item[\texttt{dummy}] not used
\item[\texttt{tilt}] tilt angle $\alpha_\mathrm{tilt}$ of conical side wall of aunit cone
...
The basic formula for the form factor of a cone with 6-fold symmetry (blunted hexagonal pyramid) has been taken from \cite{Renaud2009}. The difference from that version is that the affine transformation from above is used to scale the size of the cone. This is also the reason why the ratio of the height and radius $H_R=\frac{H}{R}$ is used as an input parameter instead of $H$ and $R$ separately; i.e., a unit 6-fold symmetric cone is assumed to have a radius of $R=1$. The overall size is then scaled with the input parameters $a$, $b$ and $c$. The shape does not have inversion symmetry and is therefore a complex function. The phase factor is taken so that the base plane lies in the $xy$-plane according to Fig.\ \ref{fig:opo_cone6}. The form factor of the cone according to \cite{Renaud2009} is given as
...
\subsection{oriented and random oriented square basis pyramid} ~\\
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.6\textwidth]{../images/form_factor/oriented_primitive_opbjects/Octahedron.png}
\end{center}
\caption{sketch of a regular octahedron  composed of two regular pyramids with edge lengths of $2a$}
\label{fig:opo_octahedron}
\end{figure}
The form factor of a regular octahedron \cite{Li2011} with an edge length of $\sqrt{2}$  reads as
\begin{align}\label{eq:FF_RegularOctahedron}
  F_\mathrm{rOH}(\mathbf{Q}) &= \frac{V_\mathrm{rOH}}{Q_x^2-Q_y^2} \left(h(Q_y,Q_z)+h(Q_z,Q_x)\right)\\
  V_\mathrm{rOH} &= \frac{4}{3} \\
  h(Q_1,Q_2) &= 8\frac{Q_1\sin Q_1 - Q_2\sin Q_2}{Q_1^2-Q_2^2}
\end{align}
To be consistent with the length definition of the other oriented primitive objects having an half axis length of $a$, $b$, and $c$ we can use $\hat{\mathbf{Q}}$ as defined in \ref{eq:Q:dach} and $\det(\M{D}^{-1})$ as defined in \ref{eq:detDm1} to also include an affine deformation and finally get
\begin{align}\label{eq:FFOctahedron}
  F_\mathrm{OH}(\mathbf{Q}) &=  \left(\eta_p-\eta_m\right) \frac{\det(\M{D}^{-1})}{2\sqrt{2}}F_\mathrm{rOH}(\hat{\mathbf{Q}}/\sqrt{2}) \\
  P_\mathrm{OH}(\mathbf{Q}) &=F_\mathrm{OH}^2(\hat{\mathbf{Q}})
\end{align}
with $\eta_p$ being the scattering length density of the particle and $\eta_m$ that one of the matrix.

For a random oriented oblique octahedron the orientation average is done over all directions of $\B{Q}$. The corresponding scattering amplitude $F_{\mathrm{OH},rnd}(Q)$ and scattering intensity $P_{\mathrm{OH},rnd}(Q)$ are then calculated in the same way as in eqs.\ \ref{eq:opo_rnd_cylF} and \ref{eq:opo_rnd_cylF2}.

\begin{figure}[htb]
\includegraphics[width=0.31\textwidth]{../images/form_factor/oriented_primitive_opbjects/octahedron_0_0_0.pdf} \hfill
\includegraphics[width=0.31\textwidth]{../images/form_factor/oriented_primitive_opbjects/octahedron_0_90_0.pdf}  \hfill \includegraphics[width=0.31\textwidth]{../images/form_factor/oriented_primitive_opbjects/octahedron_0_0_90.pdf}
\caption{2D scattering patterns of an oriented octahedron. The octahedrons are simulated with half axis length of $a=1.5b=5c$. The Tait–Bryan angles (yaw-pitch-roll) are $(\alpha=0^\circ,\beta=0^\circ,\gamma=0^\circ)$, $(\alpha=0^\circ,\beta=90^\circ,\gamma=0^\circ)$, and $(\alpha=0^\circ,\beta=0^\circ,\gamma=90^\circ)$ }
\label{fig:opo_coneIQ2D}
\end{figure}

~\\
\uline{Input Parameters for model \texttt{octahedron (opo)}:}
\begin{description}
\item[\texttt{a}] length of first half axis
\item[\texttt{ea\_x}] $x$-component of fist axis.
\item[\texttt{ea\_y}] $y$-component of fist axis.
\item[\texttt{ea\_z}] $z$-component of fist axis.
\item[\texttt{nu}] length of second half axis $b=\nu a$
\item[\texttt{eb\_x}] $x$-component of second axis.
\item[\texttt{eb\_y}] $y$-component of second axis.
\item[\texttt{eb\_z}] $z$-component of second axis.
\item[\texttt{mu}] length of third half axis $c=\mu a$
\item[\texttt{ec\_x}] $x$-component of third axis.
\item[\texttt{ec\_y}] $y$-component of third axis.
\item[\texttt{ec\_z}] $z$-component of third axis.
\item[\texttt{eta\_p}] scattering length density $\eta_p$ of particle
...
\begin{figure}[htb]
\begin{center}
\includegraphics[width=0.7\textwidth]{../images/form_factor/oriented_primitive_opbjects/octahedronIQ.pdf}
\end{center}
\caption{Scattering curves of an octahedron compared to a sphere of the same volume of $10^4$nm$^3$.}
\label{fig:opo_octahedronIQ}
\end{figure}
~\\
\uline{Input Parameters for model \texttt{octahedron (opo, random)}:}
\begin{description}
\item[\texttt{a}] length of first half axis
\item[\texttt{ea\_x}] $x$-component of fist axis.
\item[\texttt{ea\_y}] $y$-component of fist axis.
...
\begin{itemize}
\item the unit vector $\B{e}_a$, $\B{e}_b$, and $\B{e}_c$ are internally normalized to 1 and need to be linear independent.
\item the volume of the oblique octahedron is $V_\mathrm{OH}=\frac{\sqrt{2}}{3}8\det(\M{D}^{-1})$
\end{itemize}
...
\uline{Input Parameters for model \texttt{superellipsoid (OPO)}:}
\begin{description}
\item[\texttt{a}] length of first half axis
\item[\texttt{ea\_x}] $x$-component of fist axis.
\item[\texttt{ea\_y}] $y$-component of fist axis.
...
~\\
\uline{Input Parameters for model \texttt{superellipsoid (OPO, random)}:}
\begin{description}
\item[\texttt{a}] length of first half axis
\item[\texttt{ea\_x}] $x$-component of fist axis.
\item[\texttt{ea\_y}] $y$-component of fist axis.
...
\item shape parameter $\epsilon_2$ needs to be a positive non-zero number, $\epsilon_2>0$
\item The integration strategy for the double integral in eq.\ \ref{eq:superellipsoid_IQ} can be configured via a menu interface as well as the integration routine for the random orientational average.
\end{itemize}
