...
\end{align}
where $n$ is the number of particles per unit cell, $f_{hkl}$ is the symmetry factor taking into account extinction rules, $v_d$ is the volume ($d=3$), surface ($d=2$), or long-period ($d=1$) of the $d$-dimensional unit cell, $\Omega_d$ is the $d$-dimensional solid angle, and $L_{hkl}(Q-Q_{hkl})$ is a normalised peak-shape function. The sum is done over all reflections $\{hkl\}$ where $m_{hkl}$ is the corresponding multiplicity. For the normalized peak shape function $L_{hkl}(x)$ the user can choose between Lorentzian, Gaussian, and Pearson VII peak shape
\begin{align}
L_{hkl}(x) &=
\begin{cases}
...
$$
\mathbf{Q}_{hkl} = h\mathbf{a}^\star+k\mathbf{b}^\star+l\mathbf{c}^\star
$$
Due to the symmetry of the unit cell certain $(hkl)$ reflections might be forbidden. This is described by the structure factor of the unit cell $F_{hkl}$. The structure factor depends next to the Miller indices also from the type and position of the scattering objects within the unit cell. The position $\mathbf{R}_i(u,v,w)$ of the $i^\mathrm{th}$ scatterer with the unit cell are normally given in terms of the direct lattice vectors
$$
\mathbf{R}_i(u_i,v_i,w_i) = u_i \mathbf{a} + v_i\mathbf{b} +w_i\mathbf{c}
$$
The scattering amplitude of the unit cell $F_{uc}$ is than given by
\begin{align}
F_{uc} (\mathbf{Q}_{hkl}) &= \sum_{i=1}^N f_i(\mathbf{Q}_{hkl}) \exp\left( -\imath \mathbf{Q}_{hkl} R_i\right) \\
                          &= \sum_{i=1}^N f_i(\mathbf{Q}_{hkl}) \exp\left( -\imath 2\pi \left(hu_i+kv_i+lw_i\right)\right) \\
                          &= \sum_{i=1}^N f_i(\mathbf{Q}_{hkl}) \cos\left( 2\pi \left(hu_i+kv_i+lw_i\right)\right) - \imath \sin\left( 2\pi \left(hu_i+kv_i+lw_i\right)\right)
\end{align}
...
\begin{tabular}{|>{\columncolor[gray]{1.0}[0.8\tabcolsep][0.8\tabcolsep]} l%
 ...
 $d$ & 3 & 3 & 3 & 3 & 3\\
 \rowcolor[gray]{0.95}
 $ f_{hkl}$ & $\scriptscriptstyle f_{hkl}= 1+\cos(\pi(h+k+l))$ &
    $\scriptscriptstyle \begin{array}{r@{}l@{}} \scriptscriptstyle f_{hkl}=& {}\scriptscriptstyle 1+\cos(\pi(h+k)) \\&{}\scriptscriptstyle+\cos(\pi(h+l)) \\&{}\scriptscriptstyle+\cos(\pi(k+l))\end{array}$ &
    $\scriptscriptstyle f_{hkl}=1+\cos(\pi(h+k+l))$ &
    $\scriptscriptstyle f_{hkl}=2\cos\left(\pi\left(\frac{h+2k}{3}+\frac{l}{2}\right)\right)$ &
