...
\chapter{Numerical solution of the Ornstein Zernike equations}

In \SASfit an algorithm to numerically calculate structure factors
$S(q)$, radial distribution functions $g(r)$ and direct correlation
functions $c(r)$ of a systems with known pair interaction potential
$U(r)$ has been implemented. The central part of this package is
iterative solution of Ornstein-Zernike equation
\cite{Naegele2004,Henderson2000,Hansen2013}:
...
of particle 1 on particle 2), $c(\mathbf{r}_{13})$ the indirect
correlation function (direct effect of particle 1 on particle 3
which influences particle 3), and $\rho$ the particle number
density. If the fluid is uniform and isotropic, the Ornstein–Zernike
relation becomes
...
indirect (through neighboring particles) interactions. In order so
solve the Ornstein Zernike a closure relation is required:
\begin{align}\label{eq:ozclosure}
c(r)&= \exp\left[ -\beta U(r) +\Gamma(r)+B(r)\right] - \Gamma(r)-1 \\
\mbox{or } h(r)+1&=g(r)= \exp\left[ -\beta U(r)
+\Gamma(r)+B(r)\right]
\end{align}
where $U(r)$ is the pair interaction potential, $\beta=1/(k_B T)$
the inverse temperature, $\Gamma(r)=h(r)-c(r)$ the indirect (chain)
correlation function, and $B(r)$ the bridge function. The closure is
not complete as long the bridge function is known. Even though the
bridge function is exactly defined the function is an unknown
function of interparticle distance and only approximations can be
given.
...
g(r)&=\exp\left[ -\beta U(r) +h(r)-c(r)\right]
\end{align}
...
g(r)&= \exp\left[ -\beta U(r)\right]\left(1 +h(r)-c(r)\right)
\end{align}
...
\subsection{Roger-Young closure}

\section{Phenomenological Closures}
\subsection{Verlet approximation}

\begin{align} \label{eq:ozBHNC}
B_\text{Verlet}(r) &=\frac{-\Gamma^2(r)}{2\left(1+\frac{4}{5}\Gamma(r)\right)} \\
g(r)&= \exp\left[ -\beta U(r)\right]\left(1 +h(r)-c(r)\right)
\end{align}

\subsection{Martynov-Sarkisov}

\subsection{Ballone, Pastore,
Galli, and Gazzillo (BPGG) approximations}
