...
in great detail in \cite{Naegele2004,Henderson2000,Hansen2013,Bomont2008,Caccamo1996}.
...
and Caccamo \cite{Caccamo1996} and the references they give.
...
~\\
A semi-henomenological equation for the radial distribution function was proposed by Verlet
...
\begin{align}
u^\text{LR,DH}_\text{pert}(r) &= -4k_\text{B}T\epsilon\left(\frac{r}{\sigma}\right)^6
\times \exp \left( -\frac{k_\text{B}T\epsilon}{\rho^*} \left(\frac{r}{\sigma}\right)^6 \right)
\end{align}
...
~\\
The penetrable sphere model became popular after 1989 due to Marquest and
...
\begin{cases}
k_\text{B} T \epsilon   & \mbox{if } r \leq \sigma \\
0                       & \mbox{if } r >    \sigma
\end{cases}
\end{align}
...
\noindent ~\\ The Lennard-Jones potential is a mathematically simple model that approximates the interaction
