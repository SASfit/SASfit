...
In physics, many experimental data sets can be described theoretically by a Fredholm integral. To extract a physically meaningful quantity, the integral equation often has to be solved numerically. In many cases, this problem is ill-posed, and an additional regularization technique must be included in the analysis to obtain a stable solution. A general overview of regularization techniques can be found, e.g., in \cite{Hansen1998,Hansen2000b,Gazzola2018,Scherzer2011,Kern2016}.
...
Scattering techniques also often measure a quantity that can be written in the form of a Fredholm integral.
...
\end{align}
The problem is to find $f(s)$, given the continuous kernel function $K(t,s)$ and the function $g(t)$, which is often an experimentally measured quantity with an error bar. Sometimes the kernel values scale with a power law $s^\alpha$. In such a case, a more stable solution can be obtained by rewriting the problem as
\begin{align}
\label{eq:KernelScaling}
g(t)=\int _{a}^{b}s^{-\alpha} K(t,s) \, s^{\alpha}f(s)\,\mathrm {d} s
\end{align}
where the continuous kernel function is now defined as $s^{-\alpha}K(t,s)$, and the function to be determined is $s^{\alpha} f(s)$.

The above Fredholm equation is one dimensional. There are several cases where the physical problem to solve is described by a multi-dimensional Fredholm integral of the form
\begin{align}
\label{eq:Fredholm}
...
One simple example is the smearing of a signal by a known resolution function. These folding integrals can, in principle, be solved in Fourier space. However, signals without smearing are also often described by a known or assumed function with a distributed parameter. The resulting signal is then an integral of the product of the known function and the probability distribution to be determined. In both cases, these integrals belong to the class of Fredholm integrals of the first kind.

In small-angle scattering, the measured intensity $I_m$ is given as a function of the scattering vector $q=4\pi\sin(\theta)/\lambda$, where $\theta$ is the scattering angle and $\lambda$ is the wavelength of the probing radiation. The smearing due to limited resolution for a pinhole camera can be written as
...
\end{align}
 ...
 \end{align}
For the analysis, one first has to calculate $g_{(1)}(\tau)$ from $g_2(\tau)-1$. Often, one simply takes the square root of the reduced intensity autocorrelation to obtain the amplitude autocorrelation. As the amplitude autocorrelation function is always positive, this is justified. However, due to noise, the reduced intensity autocorrelation can also become negative for large relaxation times $\tau$, as $g_2(\tau)-1=\abs{g_1(\tau)}^2 \rightarrow 0$ for $\tau\rightarrow\infty$. Simply neglecting negative reduced intensity-autocorrelation values would produce systematic errors in the analysis. A more stable and reliable strategy, although it is not well justified mathematically, is to set
\begin{align}
\label{eq:sqrt_g2m1}
 g_{(1)}(\tau) &= \mathrm{sign}(g_{(2)}(\tau)-1) \sqrt{\abs{g_{(2)}(\tau)-1}}
\end{align}
Another strategy is to make use of the convolution theorem for the Laplace transform.
...
\end{align}
Pike \cite{Pike2002} suggests first calculating $G_2(\Gamma)$, then taking the square root of its Laplace transform, and finally calculating the inverse transform of that square root.
\begin{align}
G(\Gamma) &=
...
Goldin \cite{Goldin1991} suggested a combination of both methods. He showed that, even when the value under the radical in Eq.\ \ref{eq:sqrt_g2m1} is non-negative, calculating $g_1(\tau)$ from $\sqrt{g_2(\tau)-1}$ increases the random error relative to $g_2(\tau)$ and introduces a systematic error. He proposed computing the distribution of interest in two stages. In the first stage, the self-convolution of the distribution of interest, $G_{(2)}(\Gamma)$, is computed from the function $g_2(\tau)-1$, as in \cite{Pike2002}. A smoothed estimate of $g_2(\tau)$ is then computed and used to calculate the first-order correlation function $g_1(\tau)$. This is then used to calculate $G(\Gamma)$.
...
\end{align}
Furthermore, the scattering intensity of the particle, $I(q,R_h)$, contributes as a weighting factor, i.e., it determines how much a given size contributes to the relaxation distribution function at the corresponding relaxation time $G(\Gamma)$.
\begin{align}
\label{eq:GRh}
...
\end{align}
The volume and intensity distributions $N_v(R_h)$ and $N_i(R_h)$ are then defined as
\begin{align}
N_v(R_h) &= R_h^3 N_h(R_h) \\
N_i(R_h) &= R_h^6 N_h(R_h)
\end{align}
Equations \ref{eq:Nh} and \ref{eq:GRh} show that the size distribution or decay-rate distribution also depends on the $q$- and size-dependent scattering function $I(q,R_h)$. Analysing intensity-correlation functions at different angles simultaneously normally helps to obtain a more stable solution for the particle size distribution.
...
Although the fixed point of the EM operator does not seem to depend on the initial values, it will probably depend on the noise in the data. If the curve were measured a second time with similar statistics, the two curves would differ within their noise levels, and the EM fixed point would therefore very likely be slightly different. This is a consequence of the ill-posed nature of the Fredholm integral. Two strategies for overcoming this problem have been tested: one introduces a smoothing operator into the iteration loop, and the other introduces a classical regularization term into the EM scheme. Both require the smoothing or regularization parameter to be determined.

~\\

\subsection{EM with linear and non-linear smoothing operator}~\\

Without additional smoothing, the EM scheme does not converge to a stable solution for scattering curves measured several times with the same statistics. Several stabilization methods have been suggested. One is to add a smoothing operator to the iteration sequence. In \SASfit, the smoothing operation suggested by \cite{Eggermont1999,Eggermont1995,Byrne2011} has been implemented. The authors suggest adding a smoothing step before and/or after each normal EM step (Eq.\ \ref{eq:LucyRichardsonInversionMethod}), so that the EM algorithm takes the following form:
...
In the case of double smoothing, they suggested a nonlinear smoothing operator before each EM step, so that a complete smoothed EM step reads as follows:
...
\caption{Roughness and goodness-of-fit parameters, as well as the resulting volume distribution $N(R)R^3$, as a function of the number of iterations for a constant seed value (top) and random seed values (bottom), for an optimum smoothing parameter $h=1.87\times 10^{-4}$ leading to $\chi_r^2=1$.\label{fig:EMhoptimum}}
\end{figure}
...
\caption{The smoothing parameters were obtained from the corners of the L-curves in Fig.\ \ref{fig:EMhLcurve}, and the solution of the EM iteration scheme with double smoothing is compared with the exact solution.\label{fig:EMsmoothNRR3}}
\end{figure}
...
Another efficient method for accelerating the convergence of the EM iteration scheme is Anderson acceleration \cite{Anderson1965}, which solves fixed-point problems (see also \cite{Walker2011,Toth2015}) and has also been suggested for the EM iteration scheme \cite{Henderson2019}. In \SASfit, several fixed-point acceleration methods are supplied using the SUNDIALS library \cite{Hindmarsh2005}. However, only the supplied Anderson acceleration speeds up the fixed-point iterations; the other methods, such as GMRES \cite{Saad1986}, FGMRES \cite{Saad1993}, Bi-CGStab \cite{Vorst1992}, and TFQMR \cite{Freund1993}, become unstable and do not converge.

In \cite{Biggs1997, Biggs1995, Biggs1998, Wang2014,Jiang2017}, a vector-extrapolation acceleration method was suggested that does not require an extra evaluation of the fixed-point operator $\mathcal{O}_\mathrm{EM}$ in Eq.\ \ref{eq:LucyRichardsonInversionMethod}. The method needs to retain only one or two previous solution vectors to calculate either the first derivative or, additionally, the second derivative, and to estimate the next virtual solution vector. This vector is then used as the input vector for the next fixed-point iteration. The algorithm is described in Alg.\ \ref{algorithm:Biggs_Andrews}. The convergence behaviour of the different acceleration schemes is shown in Fig.\ \ref{fig:EMconvergenceSpeed}. The Biggs-Andrews and Anderson methods provide good acceleration, whereas other classical methods, such as GMRES, Bi-CGStab, and TFQMR, failed. The Picard iteration scheme and the overrelaxation method require orders of magnitude more iterations to reach the true fixed point. The preferred acceleration scheme is therefore Biggs-Andrews acceleration.
