...
\section{$G(z)$ for a generaliyed Gaussian Coil (gGc) }~\\
The scattering intensity for a a generaliyed Gaussian Coil (gGc) has been given by given Hammouda \cite{Hammouda,Hammouda2012,Hammouda1993,Hammouda2016} as shown in section \ref{sect:generalized_gaussian_coil} as
\begin{align}
I_\text{gGc}(q) &= I_0
\left(
...
\begin{align}
\tilde{G}_\mathrm{gGc}(0) &= \frac{3 \pi }{\left(4 \nu ^4-5 \nu ^2+1\right) \text{Rg}^2} \mbox{~for~} \nu \in \left(0,\frac12\right)
\end{align}