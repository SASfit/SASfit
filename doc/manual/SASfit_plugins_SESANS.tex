...
\begin{tikzpicture}[->,>=stealth,shorten >=5pt,auto,node distance=3cm,
  ...
  \node[main node] (gamma0) at (5,15) {$\gamma_0(r)$};
  \node[main node] (gamma) at (5,8.66) {$\tilde{\gamma}(r)$};
  \node[main node] (IQ) at (10,0) {$I(Q)$};
  \node[main node] (Gz) at (0,0) {$\tilde{G}(z)$};
  ...
  \path[every node/.style={font=\sffamily\large,
    ...
    (gamma0) edge [bend right=15] node[outer sep=6pt,pos=0.5,left] {
    $\frac{\tilde{\gamma}(r)}{\Delta\eta^2}= \gamma_0(r) \int_0^\infty 4\pi r^2 \gamma_0(r) \mathrm{d}r$} (gamma)
    (gamma) edge [bend right=15] node[outer sep=6pt,pos=0.5,right] { $ \gamma_0(r)= \frac{\tilde{\gamma}(r)}{\tilde{\gamma}(0)}=\frac{\tilde{\gamma}(r)}{V_p\Delta\eta^2}$} (gamma0)
    (gamma) edge [bend right=15] node[outer sep=6pt,pos=0.5,sloped,below] { $ \mathcal{F}= (2\pi)^3\mathcal{F}^{-1}$} (IQ)
            edge [bend right=15] node[outer sep=6pt,pos=0.5,sloped,above] { $ (2\pi)^2\mathcal{A}$} (Gz)
    (Gz) edge [bend right=15] node[outer sep=6pt,pos=0.5,sloped,below] { $ \frac{1}{2\pi}\mathcal{H}^{-1}=\frac{1}{2\pi}\mathcal{H}$} (IQ)
         edge [bend right=15] node[outer sep=6pt,pos=0.5,sloped,below] { $ \frac{1}{(2\pi)^2}\mathcal{A}^{-1}$} (gamma)
    (IQ) edge [bend right=15] node[outer sep=6pt,pos=0.5,sloped,above] { $ 2\pi\mathcal{H}=2\pi\mathcal{H}^{-1}$} (Gz)
         edge [bend right=15] node[outer sep=6pt,pos=0.5,sloped,above] { $ \mathcal{F}^{-1}=\frac{1}{(2\pi)^3}\mathcal{F}$} (gamma);
\end{tikzpicture}
\end{center}
\caption{$\mathcal{FAH} $-cycle between $\gamma(r)$, $I(Q)$, and $G(z)$}
\label{fig:FAHcycle}
\end{figure}
