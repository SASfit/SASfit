...
\end{align}
In the literature, expressions for the scattering length density autocorrelation function are often normalized to 1 at $r=0$; i.e., the expressions $\gamma_0(r)=\tilde{\gamma}(r)/\tilde{\gamma}(0)$ are given. It also follows that
\begin{align}
...
\end{align}
where, in the case of a spherical particle, $V_P=\frac43 \pi R^3$.
Fig.\ \ref{fig:FAHcycle} shows how the functions $\tilde{\gamma}(r)$, $\tilde{G}(\delta)$, and $I(Q)$ are related. However, the cycle is mathematically exact only for a one-dimensional Fourier transformation of a radially symmetric function. This transformation cycle is called the projection-slice theorem and is a standard method in imaging \cite{Bracewell2003, Bracewell1956}. The projection-slice theorem makes use of the fact that
\begin{align}
...
\end{align}
with $\mathcal{F}_{1D}(s)=\int_{-\infty}^{\infty} \exp\left(-\imath 2\pi s r\right) f(r) \mathrm{d}r$, i.e., using Fourier-transform convention (1) and Hankel-transform convention (1), as given in footnotes\footref{footnote:Fourier} and \footref{footnote:Hankel}. It has been shown in \cite{Kohlbrecher2017} that the $\mathcal{FAH}$ cycle remains valid when using the isotropic three-dimensional Fourier transform $\mathcal{F}_{I,3D}$ shown in Fig.\ \ref{fig:FAHcycle}.
...
\end{center}
\caption{\SASfit allows a final transformation of the model function to convert a SANS signal into a SESANS signal.}
\label{fig:HankelOp}
\end{figure}
...
\label{sec:Gz_gGc}
The scattering intensity for a generalized Gaussian coil (gGc) was given by Hammouda \cite{Hammouda,Hammouda2012,Hammouda1993,Hammouda2016} (see also Section \ref{sect:generalized_gaussian_coil}) as
\begin{align}
I_\text{gGc}(q) &= I_0
\left(
