...
The best choice for the iteration method depends often on the problem, i.e. the type of closure and the potential and also on the volume fraction. In some tests it turned out that the simple \texttt{Picard iteration} was often the fastest for those problems with less pronounced structure factors, i.e. low volume fractions. However at larger volume fractions the \texttt{Picard iteration} scheme failed several times where e.g. the \texttt{S Iteration} scheme still got a solution. In a few case also the root finding algorithm \texttt{Broyden} was the fastest solver (e.g. Percus-Yevick closure with a hard sphere potential at a volume fractions of 0.6, or for Vompe-Martynov closure with a Lennard Jones potential with $\epsilon=1$ and a volume fraction of 0.6), but in many other cases also much slower than e.g. the \texttt{Picard iteration} scheme. As a default iteration scheme the \texttt{S iteration} has been chosen. It is reasonable fast and converges well for many problems. Other similar well behaving, but slightly slower schemes are the \texttt{Mann iteration}, \texttt{SP iteration}, \texttt{Krasnoselskij iteration}, and \texttt{S* iteration}.
