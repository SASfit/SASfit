...
\hfill
\subcaptionbox{Kratky plot of the same data as on the left. \label{fig:IQ:WormsComparison2} }{\includegraphics[width=0.48\textwidth]{../images/form_factor/polymer_semiflexible/WormsComparisonKratky.png}}
...
\clearpage
\subsection{hyperbranched polymer}
\label{sect:hyperbranched_polymers}
~\\
...
~\\
\subsubsection{random $f$-functional homopolycondensates}
...
\begin{align}\label{eq:f-functionalHomoPolym}
  I &= ?
\end{align}
where $f$ is the functionality, $\alpha$ the fraction of functionalities which have reacted and $\sigma^2$ may be defined as the mean-square distance between two units
...
~\\
\subsubsection{regular comb}
\label{sect:com_regular}
~\\
The form factor of a regular comb has been derived in \cite{Casassa1965}. The typos of the derived equations have been corrected in \cite{von_Ferber_2013}.The form factor reads as
\begin{align}\label{eq:comb_regular}
  I_\mathrm{comb,reg}(q) &=  \frac{2}{x^2}\left(A+B+C\right)\\
  A &= x-1+e^{-x\lambda}\\
  B &= \left(1-e^{-x(1-\lambda)/f} \right)\left(f+2\frac{1-e^{-x\lambda f/(f+1)}}{1-e^{x\lambda/(f+1)}}\right)\\
  C &=  \left(1-e^{-x(1-\lambda)/f} \right)^2 \\ &\phantom{=} \qquad \left(\frac{(f-1)\left(e^{x\lambda/(f+1)}-1\right)-\left(1-e^{-x\lambda(f-1)/(f+1)}\right)}{\left(1-e^{x\lambda/(f+1)}\right)^2}\right) \nonumber\\
  x & = q^2Nb^2/6 \\
  N & N_0+fn_b \\
  \lambda & = N_0/N
\end{align} 
where $b$ is the Kuhn length, $f$ the number of side branches equally spaced along the back-bone, $N$ the total number of chain (Kuhn) elements, $N_0$ chain elements in the back-bone, $n_b$ number of chain elements in the side branch.