...
\end{align}
The difference between the WE-model and the HSH-model is how the deformed tube diameter $d_\mathrm{t}$ is calculated. In the case of the WE model, the projections $d_\mu$ of $d_t$ onto the Cartesian axes are used, whereas the HSH model uses an effective angle-dependent diameter $d_\phi$.
\begin{align}
...
\begin{enumerate}
\item In one experimental geometry, the normal $\mathbf{e}_g$ (gradient direction) of the disc-shaped sample is parallel to the incoming neutron beam $\mathbf{e}_n$. To separate the deformation in the flow and vorticity directions, apertures with a sector shape and two opposite openings of 45 degrees each, as marked in the image below, are used. In this geometry, the scattering is sensitive only to the vorticity and flow directions.\\[3mm]
...
The order parameter for the Hayter-Penfold orientation distribution is calculated by first transforming the polar coordinates, with $\mathbf{z}$ as the polar axis, to a coordinate system whose polar axis points in the direction of the most probable orientation. The order parameter $S_2(\kappa)$ in Eq.\ \ref{eq:S2kappa} is then calculated in this new polar coordinate system.

The form factors additionally include a size distribution, allowing them to benefit from the speed enhancement of a specialized multidimensional integration routine. The size distribution is included as
\begin{align}
