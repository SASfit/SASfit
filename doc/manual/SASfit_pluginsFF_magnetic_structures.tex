...
The different form-factor versions implemented in the plugin
differ in the model used to describe the scattering of the
molecules on the surface and in how the data are pre-treated. At the
moment, three different models for the polymer
chains on the surface are implemented, namely that the chains are
are noninteracting and follow random-walk behaviour \texttt{(Chain,RW)};
the second are interacting due to the high grafting density, for which
a self-avoiding-walk model \texttt{(Chain,SAW)} is applied;
thirdly a parabolic scattering length density profile for the outer
shell is used \texttt{(Chain,parabolic)}. For each model, a version
for a sector cut in the direction $\psi$ (\texttt{psi}), a radially
...
(\texttt{cross-term (rad.~avg.)}) have been implemented. Versions for
individual angle-dependent cross-sections with full polarisation
analysis are also available. The possible combinations are:
